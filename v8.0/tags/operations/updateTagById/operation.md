...
**Operation:** `updateTagById` — Update tag by ID
...
- `id` (query, string, required) [maxLen=19]: Specify the unique ID of the tag to update.
- `id` (path, string, required) [maxLen=255]: Specify the unique ID of the tag.

**Request Body** — application/json `PuttagsRequest`
> The request body must contain a **tags** array with one tag object specifying the updated **name** and optional **color_code**.
  > Represents the request body schema for the update tag operation.
  - `tags` (array of object `TagsNested`) [maxItems=1] **REQ** — Represents an array of tag objects to update. The array can contain a maximum of one item.
    schema: `TagsNested`
...
- **200**: Returns a success response for a successful tag update. The response includes the updated tag details. — Schema: `PuttagsResponse200` [application/json]
    > Represents the response body returned when a tag update operation completes successfully.
    schema: `PuttagsResponse200`
    - `tags` (array of object `TagsNested1`) [maxItems=1] — Represents an array containing the result of the tag update operation.
      schema: `TagsNested1`
...
- **400**: Returns an error response when the request contains invalid data or the tag is not found.
**Resolution:** Check the error code and verify the tag ID and field values are correct. [application/json]
    > Represents one of the possible error responses returned for a 400 Bad Request.
    oneOf:
      - `RequiredIdParamError` — Represents an error response returned when the required **ID** parameter is missing from the request.
        ...
        - `message` (string) **REQ** [enum=['One of the expected param is missing']] — Represents the error message indicating the ID parameter is missing.
      ...
      - `ModuleTagsNotSupportedError` — Represents an error response returned when the specified module does not support tags.
        ...
        - `status` (string) **REQ** [enum=['error']] — Represents the status of the error response. The value is always **error**.
        - `tags` (array of object) [maxItems=25] **REQ** — Represents an array of error objects for the invalid tag update request.
          oneOf:
...
- **403**: Returns an error response when the user does not have permission to update tags.
**Resolution:** Ensure the access token includes the **ZohoCRM.settings.tags.UPDATE** scope. — Schema: `NoTagPermissionError` [application/json]
