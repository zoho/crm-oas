...
**Operation:** `createBusinessHours` — Business Hours Configuration
...
**Request Body** (required) — application/json
  ...
  - `business_hours` (object) **REQ** — The business hours configuration object containing all settings for the organization's working schedule.
    ...
    - `custom_timing` (array of object) [maxItems=7] — Different business hours for different days when same_as_everyday is false
      ...
      - `business_timing` (array of string) [minItems=2, maxItems=2] **REQ** — Start and end times for this specific day in 24-hour HH:MM format
        items: [maxLen=5, pattern=^([0-1][0-9]|2[0-3]):[0-5][0-9]$]

**Responses:**

- **201**: Business hours operation completed successfully. [application/json]
...
- **400**: The request could not be processed due to invalid data, missing required fields, business rule violations, or an attempt to create multiple business hours configurations . — Schema: `BusinessHoursErrorResponses` [application/json]
    oneOf:
