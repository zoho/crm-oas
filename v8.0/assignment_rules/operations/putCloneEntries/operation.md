...
**Operation:** `putCloneEntries` — Clone Assignment Rule Entries
...
**Request Body** — application/json `RequestBodySchemaForActionCloneEntries`
> The request body must contain an **assignment_rules** array with one object. Specify the rule entries to clone into the target Assignment Rule.
  > Specifies the request structure for cloning rule entries in an Assignment Rule.
  - `assignment_rules` (array of object) [minItems=1, maxItems=1] **REQ** — Specify the **assignment_rules** array that contains the rule entries to clone into the target Assignment Rule.
    - `rule_entries` (array of object `RuleEntryUpdateRequestObject`) [minItems=1, maxItems=1] **REQ** — Specify the list of rule entries to clone while retaining their order and configuration in the target Assignment Rule.
      schema: `RuleEntryUpdateRequestObject`
...
**users** - Represents users.
            - `resources` (array of object `UserResourceRequestObject`) [minItems=1, maxItems=50] **REQ** — Defines the list of users details.
              schema: `UserResourceRequestObject`
