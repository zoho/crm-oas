...
**Operation:** `cloneAssignmentRule` — Clone Assignment Rule
...
**Request Body** — application/json `RequestBodySchemaForActionClone`
> The request body must contain an **assignment_rules** array with one object. Specify the name to assign to the cloned Assignment Rule.
  ...
  - `assignment_rules` (array of object) [minItems=1, maxItems=1] **REQ** — Array of Assignment Rule objects to be created.
    - `name` (string) **REQ** [maxLen=100, minLen=1] — Specify the name to be displayed for the Assignment Rule.

**Responses:**

- **201**: Returns the success details for the cloned Assignment Rule, including the new Assignment Rule ID. — Schema: `SuccessResponse` [application/json]
...
- **400**: Invalid request error cases.\n**Resolution:** The request must include a valid **module** query parameter, a valid Assignment Rule ID in the path, and a valid **assignment_rules** array with the required **name** field. [application/json]
    > Represents the possible error responses when the clone Assignment Rule request contains invalid or missing data.
    oneOf:
...
**error** - Represents error.
      - `ErrorResponseInvalidData` — Invalid data.
        - `assignment_rules` (array of object) [maxItems=25] **REQ** — Represents the list of per-rule error objects for the Assignment Rules that encountered validation failures during the clone operation.
          oneOf:
...
- **403**: Forbidden request error cases.\n**Resolution:** The CRM administrator must grant the required Assignment Rule management permission for the specified module, and the OAuth token must include the required scope. — Schema: `Possible403ErrorResponsesInManagingAR` [application/json]
...
**Scopes:** ZohoCRM.settings.assignment_rules.CREATE
