# Examples: getTerritoryById

**GET /settings/territories/{id}**

## Response examples

### Status `200` — `application/json` — TerritoryByIDSuccessResponse

Get territory data by territory ID

```json
{
  "territories": [
    {
      "created_time": "2023-05-29T16:00:54+05:30",
      "permission_type": "read_only",
      "modified_time": "2025-11-25T18:44:17+05:30",
      "manager": null,
      "account_rule_criteria": {
        "comparator": "equal",
        "field": {
          "api_name": "Account_line_1",
          "id": "809216000000711017"
        },
        "type": "value",
        "value": "432432"
      },
      "name": "rew",
      "api_name": "rew",
      "modified_by": {
        "name": "Deiva Vigneshwari Deiva Vigneshwari",
        "id": "809216000000299001"
      },
      "description": "",
      "lead_rule_criteria": {
        "comparator": "equal",
        "field": {
          "api_name": "Annual_Revenue",
          "id": "809216000000000581"
        },
        "type": "value",
        "value": "423"
      },
      "id": "809216000009380013",
      "reporting_to": {
        "name": "Zoho89dewvvew",
        "id": "809216000000306373"
      },
      "created_by": {
        "name": "Deiva Vigneshwari Deiva Vigneshwari",
        "id": "809216000000299001"
      }
    }
  ]
}
```

### Status `400` — `application/json` — TerritoryNotYetEnabled

Territory Management not yet enabled error

```json
{
  "code": "FEATURE_NOT_ENABLED",
  "details": {},
  "message": "Territory Management is not enabled",
  "status": "error"
}
```

### Status `400` — `application/json` — TerritoryDisabled

Territory Management disabled error

```json
{
  "code": "FEATURE_NOT_ENABLED",
  "details": {},
  "message": "Territory Management is disabled",
  "status": "error"
}
```

### Status `401` — `application/json` — AuthenticationFailure

Authentication failure

```json
{
  "code": "AUTHENTICATION_FAILURE",
  "details": {},
  "message": "Authentication failed",
  "status": "error"
}
```
