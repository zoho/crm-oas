...
**Operation:** `getTerritoryById` — Get territory by ID
> To retrieve the details of a specific territory in your Zoho CRM organization using its unique ID.

**Tags:** Territories

**Parameters:**
- `id` (path, string/int64, required): The unique ID of the territory to retrieve.
- `include` (query, string, optional) [maxLen=100, enum=[7 values]]: Specify the additional details to include in the response, such as **account_rule_criteria**, **lead_rule_criteria**, or **deal_rule_criteria**.
- `include_inner_details` (query, string, optional) [enum=['manager.zuid,manager.status', 'manager.zuid', 'manager.status']]: Specifies additional manager details to include, such as ZUID and account status.
...
- **200**: Returns the details of the specified territory, including its name, manager, permission type, rule criteria, and audit timestamps.
 — Schema: `TerritoryGetByIdResponse` [application/json]
    > Represents the response returned when retrieving a territory by its unique ID.
    schema: `TerritoryGetByIdResponse`
    - `territories` (array of object `TerritoryGETSchema`) [minItems=1, maxItems=1] **REQ** — Represents the list of territory objects returned for the specified ID.
...
- **204**: No territory found for the specified territory ID.

- **400**: Territory Management is not enabled for this organization.
**Resolution:** The CRM administrator must enable Territory Management in the organization settings. — Schema: `TerritoryFeatureNotEnabledError` [application/json]
