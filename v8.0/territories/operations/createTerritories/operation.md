...
**Operation:** `createTerritories` — Create Territories
...
> The request body must contain a **territories** array. You can include a maximum of 100 territory objects per request.
  ...
  - `territories` (array of object) [minItems=1, maxItems=100] **REQ** — Specify the list of territories to create.
    - `manager` (object)
      oneOf:
          - `id` (string) **REQ** [maxLen=19] — Specify the unique ID of the user to assign as the territory manager.
          type: null [const=None] — Represents the absence of a territory manager. Set this value to remove any assigned manager from the territory.
    - `reporting_to` (object) **REQ**
      oneOf:
          - `id` (string/int64) **REQ** — Specify the unique ID of the parent territory.
          type: null [const=None] — Represents the absence of a parent territory. Set this field to **null** when the territory has no parent in the hierarchy.
    - `permission_type` (string) **REQ** [enum=['read_write_delete', 'read_only']] — Specify the access permission for users in this territory.
Possible values:
read_write_delete - Grants users read, write, and delete access to records assigned to the territory.
read_only - Grants users read-only access to records assigned to the territory.
    - `description` (string) [maxLen=250, nullable] — Specify the description for the territory.
    - `account_rule_criteria` (object) — Specify the criteria for auto-assigning accounts to the territory. To remove an existing criteria, set this field to **null**.
      oneOf:
        - `SimpleFilterCriterionRequest` — Represents a simple field-based filter criterion that dispatches to the appropriate comparator-specific schema. The equal and not_equal comparators support scalar or array values; text comparators support scalar or array strings; ordering comparators require a scalar; between and not_between comparators require a two-element array; in and not_in comparators require an array of primitives; and include_all, include_any, exclude_all, and exclude_any comparators require an array of lookup objects and apply only when the field's data_type is multiuserlookup or multiselectlookup.
        - `GroupedFilterCriterionRequest` — Represents a grouped filter criterion containing multiple filter conditions evaluated with a logical operator. Supports recursive nesting for complex filtering logic.
          type: null [const=None] — Represents the absence of an account rule criteria. Set this value to remove any existing account rule criteria from the territory.
    - `deal_rule_criteria` (object) — Specify the criteria for auto-assigning deals to the territory. To remove an existing criteria, set this field to **null**.
      oneOf:
        - `SimpleFilterCriterionRequest` — Represents a simple field-based filter criterion that dispatches to the appropriate comparator-specific schema. The equal and not_equal comparators support scalar or array values; text comparators support scalar or array strings; ordering comparators require a scalar; between and not_between comparators require a two-element array; in and not_in comparators require an array of primitives; and include_all, include_any, exclude_all, and exclude_any comparators require an array of lookup objects and apply only when the field's data_type is multiuserlookup or multiselectlookup.
        - `GroupedFilterCriterionRequest` — Represents a grouped filter criterion containing multiple filter conditions evaluated with a logical operator. Supports recursive nesting for complex filtering logic.
          type: null [const=None] — Represents the absence of a deal rule criteria. Set this value to remove any existing deal rule criteria from the territory.
    - `lead_rule_criteria` (object) — Specify the criteria for auto-assigning leads to the territory. To remove an existing criteria, set this field to **null**.
      oneOf:
        - `SimpleFilterCriterionRequest` — Represents a simple field-based filter criterion that dispatches to the appropriate comparator-specific schema. The equal and not_equal comparators support scalar or array values; text comparators support scalar or array strings; ordering comparators require a scalar; between and not_between comparators require a two-element array; in and not_in comparators require an array of primitives; and include_all, include_any, exclude_all, and exclude_any comparators require an array of lookup objects and apply only when the field's data_type is multiuserlookup or multiselectlookup.
        - `GroupedFilterCriterionRequest` — Represents a grouped filter criterion containing multiple filter conditions evaluated with a logical operator. Supports recursive nesting for complex filtering logic.
          type: null [const=None] — Represents the absence of a lead rule criteria. Set this value to remove any existing lead rule criteria from the territory.
    - `name` (string) **REQ** [maxLen=50] — Specify the name of the territory.

**Responses:**

- **201**: Returns the result of the territory creation request for each territory in the **territories** array. The **code** field indicates **SUCCESS** for created territories and **ERROR** for failed ones, with the territory ID in the **details** object for successful entries. — Schema: `TerritoryCreateSuccessResponse` [application/json]
    > Represents the response returned when one or more territories are successfully created.
    schema: `TerritoryCreateSuccessResponse`
    - `territories` (array of object) [minItems=1, maxItems=100] **REQ** — Represents the list of territory objects created by the request.
      - `code` (string) **REQ** [enum=['SUCCESS']] — Represents the result code for the territory creation. Possible values: **SUCCESS**.
      - `details` (object) **REQ** — Represents additional details about the created territory, including its unique identifier.
        - `id` (string/int64) **REQ** — Represents the unique identifier of the created territory.
      - `message` (string) **REQ** [maxLen=100, enum=['Territory Created Successfully']] — Represents the success message returned for the territory creation. Possible values: **Territory Created Successfully**.
      - `status` (string) **REQ** [enum=['success']] — Represents the status of the territory creation operation. Possible values: **success**.

- **400**: The territory creation request contains invalid or missing data. Resolution: All required fields must be provided, field values must not exceed the allowed length or limits, and the permission type must be valid. [application/json]
    > Represents the error response for an invalid or failed territory creation request.
    oneOf:
      ...
      - `MandatoryNotFoundError` — Represents the error response when a mandatory field is missing from the territory request.
        - `territories` (array of object) [minItems=1, maxItems=100] **REQ** — Represents the list of territory objects with error details.
          ...
          - `status` (string) **REQ** [enum=['error']] — Represents the status of the operation. Possible values: **error**.
      - `MandatoryNotFoundErrorWithSupportedValues` — Represents the error response when a required field is missing from the request body, including the list of valid options.
        - `territories` (array of object) [minItems=1, maxItems=100] **REQ** — Represents the list of territory objects with error details.
          - `code` (string) **REQ** [enum=['MANDATORY_NOT_FOUND']] — Represents the error code for the territory error. Possible values: **MANDATORY_NOT_FOUND**.
          - `details` (object) **REQ** — Represents additional details about the error, including the field name, JSON path, and list of accepted values.
            - `api_name` (string) **REQ** [maxLen=100, minLen=1] — Represents the API name of the field associated with the validation error.
            - `json_path` (string) **REQ** [maxLen=250, minLen=1] — Represents the JSON path of the field in the request body that caused the validation error.
            - `supported_values` (array of string) [minItems=1, maxItems=10] **REQ** — Represents the list of accepted values for the field.
              items: [maxLen=100]
          - `message` (string) **REQ** [enum=['required field not found']] — Represents the error message returned when a required field is absent. Possible values: **required field not found**.
          - `status` (string) **REQ** [enum=['error']] — Represents the status of the operation. Possible values: **error**.
      - `TerritoryMaxLimitExceededErrorSchema` — Represents the error response when the number of territories in the request exceeds the allowed maximum.
        - `territories` (array of object) [minItems=1, maxItems=100] **REQ** — Represents the list of territory objects with limit exceeded error details.
          - `code` (string) **REQ** [enum=['LIMIT_EXCEEDED']] — Represents the error code for the request. Possible values: **LIMIT_EXCEEDED**.
          - `details` (object) **REQ** — Represents additional details about the error, including the maximum allowed limit.
            - `limit` (integer/int32) **REQ** [min=1] — Represents the maximum number of territories allowed per request.
          - `message` (string) **REQ** [maxLen=100, enum=['Territory Limit reached']] — Represents the error message returned when the territory limit is exceeded. Possible values: **Territory Limit reached**.
          - `status` (string) **REQ** [enum=['error']] — Represents the status of the operation. Possible values: **error**.
      ...
      - `InvalidDataWithMaximumLength` — Represents the error response when a field value exceeds the maximum allowed length.
        - `territories` (array of object) [minItems=1, maxItems=10] **REQ** — Represents the list of territory objects with error details.
          ...
          - `status` (string) **REQ** [enum=['error']] — Represents the status of the operation. Possible values: **error**.
      - `InvalidDataWithMaximumLengthAsString` — Represents the error response when a field value exceeds the maximum allowed character length.
        - `territories` (array of object) [minItems=1, maxItems=10] **REQ** — Represents the list of territory objects with error details.
          - `code` (string) **REQ** [enum=['INVALID_DATA']] — Represents the error code for the territory error. Possible values: **INVALID_DATA**.
          - `details` (object) **REQ** — Represents additional details about the error, including the field name, JSON path, and maximum allowed length.
            - `api_name` (string) **REQ** [maxLen=100, minLen=1] — Represents the API name of the field whose value exceeds the maximum allowed length.
            - `json_path` (string) **REQ** [maxLen=250, minLen=1] — Represents the JSON path of the field in the request body whose value exceeds the maximum allowed length.
            - `maximum_length` (string) **REQ** [const=1] — Represents the maximum number of characters allowed for the field, returned as a string.
          - `message` (string) **REQ** [enum=['invalid data', 'the value of the field exceeds the maximum allowed length']] — Represents the error message returned when a field value exceeds the maximum allowed length. Possible values: **invalid data**, **the value of the field exceeds the maximum allowed length**.
          - `status` (string) **REQ** [enum=['error']] — Represents the status of the operation. Possible values: **error**.
...
- **401**: Authentication failure due to missing, expired, or invalid OAuth token. Resolution: A new access token must be generated with the required scope for this API. — Schema: `UnauthorizedError` [application/json]
