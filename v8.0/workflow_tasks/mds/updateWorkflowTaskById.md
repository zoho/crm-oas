...
**Operation:** `updateWorkflowTaskById` — Automation Task
...
**Parameters:**
- `id` (path, string, required) [maxLen=20]: The unique ID of the automation task definition to update (not a Task record ID).  Use the [Workflow Tasks API](workflow_tasks.yaml#$.paths./settings/automation/tasks.get) to retrieve the ID.


**Schemas:**
`AuthorizationError`:
  > Locked task cannot be edited
  - `code` (string) **REQ** [enum=['AUTHORIZATION_ERROR']] — Error code
...
- **200**: Task updated successfully. Returns the updated task ID. — Schema: `UpdateTaskSuccessResponse` [application/json]
    > Success response for updating an automation task. Contains the updated task ID.
    schema: `UpdateTaskSuccessResponse`
    - `tasks` (array of object `TaskActionResult`) [maxItems=1] **REQ** — Array containing exactly one result for the updated task.
...
- **400**: Validation error in request payload. Two response shapes exist: (1) Request-structure errors (e.g., invalid task ID, read-only task, invalid module type/length, missing field api_name) return tasks as a single error object. (2) Per-task validation errors (e.g., field_mapping errors, dependent mismatch, mandatory not found) return tasks as an array with one error object per task entry. Parse tasks as either object or array to handle both shapes. [application/json]
    > Error response returned when an automation task update fails validation, covering request-structure errors, per-task field-mapping errors, dependent-field mismatches, and missing mandatory fields.
    oneOf:
        - `tasks` (array of object) [maxItems=25] **REQ** — Array of error objects
          oneOf:
            ...
            - `AuthorizationError` — Locked task cannot be edited
      ...
      - `TaskNotAllowedSchema` — Returned when the requested operation is not allowed  - such as deleting a task associated with an Approval Process, Workflow Rule, or Blueprint, or modifying a read-only system-managed task.
        - `tasks` (object) **REQ** — Represents the single error object returned when the update fails due to a request-structure issue such as an invalid module data type or a missing field api_name.
          oneOf:
            - `TaskMandatoryNotFoundSchema` — Returned when a required field or parameter is missing from the request payload.
            - `TaskInvalidDataSchema` — Returned when the request contains invalid data  - such as an unrecognized field, wrong data type, value exceeding maximum length, unsupported parameter value, or invalid resource ID.

- **401**: Locked or system-managed task cannot be edited. The task is protected by a workflow rule, Approval Process, or Blueprint lock. The AUTHORIZATION_ERROR is returned inside a tasks array wrapper. Note: Unlike standard HTTP 401 semantics, this response indicates the task is locked by a workflow rule, Approval Process, or Blueprint, not an authentication failure. [application/json]
    > Wrapped authorization error response for locked tasks.
    - `tasks` (array of object `AuthorizationError`) [maxItems=1] **REQ** — Array containing the authorization error.

- **403**: Insufficient permission, feature not available, or task is read-only. [application/json]
    > Error response returned when the caller is denied permission to update the automation task, either due to profile-level restrictions or because the task is read-only.
    oneOf:
      - `TaskNoPermissionSchema` — Returned when the caller does not have the required permissions to perform this operation or access this feature.
