...
**Operation:** `getWorkflowTasks` — List automation task definitions with optional filters
...
**Parameters:**
- `filter` (query, string, optional) [maxLen=1000]: Filter object to search tasks by name. Supported shape: field.api_name must be 'name', comparator must be 'contains', and value is the search string. Example: {"field":{"api_name":"name"},"comparator":"contains","value":"follow up"}
- `feature_type` (query, string, optional) [enum=[15 values]]: Filter tasks by automation feature type. The filter accepts more values than POST does - values like `blueprint_transition`, `review_process`, and `orchestration` are system-created task types that can be retrieved but not created via POST. For POST, only `workflow`, `approval_process`, `Blueprint`, `kiosks`, `scoring_rule`, and `case_escalation_rule` are valid.
- `module` (query, string, optional) [maxLen=100]: Filter tasks by CRM module API name (for example: Leads, Contacts, Deals).
- `per_page` (query, integer/int32, optional) [min=1, max=200, default=200]: Specify how many tasks to return per page. The default and the maximum possible value is 200.
- `page` (query, integer/int32, optional) [min=1, default=1]: To get the list of tasks from the respective pages. The default value is 1. Possible values: Positive integer values only.
- `sort_by` (query, string, optional) [enum=['modified_time']]: Sort field. Currently only modified_time supported.
- `sort_order` (query, string, optional) [enum=['desc', 'asc']]: Sort order for the results based on sort_by. Possible values: desc, asc.
...
- **200**: Successful retrieval of a page of automation tasks. The info object conveys pagination state (page, per_page, count, more_records). — Schema: `GetTasksSuccessResponse` [application/json]
    > Paginated response containing automation tasks and pagination metadata.
    schema: `GetTasksSuccessResponse`
    - `tasks` (array of object `TaskDetails`) [maxItems=200] **REQ** — List of automation task definitions for the current query page.
      schema: `TaskDetails`
...
**emailandpopup** — delivers both simultaneously.
    ...
    - `info` (object `PaginationInfo`) **REQ** — Details about pagination for resource listings, such as count, page number, and if more records are available.
      schema: `PaginationInfo`
      - `per_page` (integer/int32) — Maximum number of records/actions requested per page.
      - `count` (integer/int32) — Number of records/actions returned in the current response page.
      - `page` (integer/int32) — Current page number in the paginated result set (typically reflects the requested page value).
      - `more_records` (boolean) — Indicates whether additional records/actions are available beyond the current page.

- **204**: No automation tasks match the supplied filters or no tasks exist in the organization.

- **400**: Invalid query parameter (module or feature_type), or the feature is not allowed. — Schema: `GetTasksListErrorResponse` [application/json]
    > Error response for invalid GET list query parameters.
    oneOf:
...
- **401**: Authentication failed or session expired. — Schema: `PostTasksAuthenticationErrorResponse` [application/json]
...
- **403**: No permission to access workflow tasks — Schema: `TaskNoPermissionSchema` [application/json]
