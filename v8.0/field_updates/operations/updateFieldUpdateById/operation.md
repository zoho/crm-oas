...
**Operation:** `updateFieldUpdateById` — Update Field Update
...
`InvalidDataGenericError`:
  > Returned when the field_updates value has an invalid data type.
...
`UnsupportedKeyNotAllowedError`:
  > Returned when the request contains keys that are not supported for the selected field update configuration.
...
> Request body containing exactly one field update configuration to modify.
...
- **200**: Field update action updated successfully. — Schema: `UpdateFieldUpdateSuccessResponse` [application/json]
    > Success response payload returned after a field update action is modified.
    schema: `UpdateFieldUpdateSuccessResponse`
    - `field_updates` (array of object `FieldUpdateActionResult`) [maxItems=1] — Array containing the updated field update action. Contains exactly one item.
...
- **400**: Validation error. Returned when one or more request fields are missing, invalid, mismatched with dependencies, or not allowed for the selected field update configuration. [application/json]
    oneOf:
      ...
      - `InvalidFieldUpdateIdError` — Represents an invalid field update ID.
        ...
        - `status` (string) **REQ** [enum=['error']] — Error status
      - `MissingFieldUpdateIdError` — Indicates that no field update ID was provided for the edit operation.
      ...
      - `UpdateFieldUpdateMissingPathIdError` — Returned when the field update ID path segment is missing in the update request URL.
        - `code` (string) **REQ** [enum=['REQUIRED_PARAM_MISSING']] — Represents the error code.
        - `details` (object) **REQ** — Error details identifying the missing path segment
          - `resource_path_index` (integer/int32) — Index of the missing resource path segment.
        - `message` (string) **REQ** [maxLen=255] — Represents the error message.
        - `status` (string) **REQ** [enum=['error']] — Error status
      - `UpdateFieldUpdateReadOnlyActionError` — Returned when attempting to edit a read-only field update action.
        - `code` (string) **REQ** [enum=['NOT_ALLOWED']] — Represents the error code.
        - `details` (object) **REQ** — Error details
          - `resource_path_index` (integer/int32) — Index of the resource path used in the request.
        - `message` (string) **REQ** [maxLen=255] — Represents the error message.
        - `status` (string) **REQ** [enum=['error']] — Error status
...
- **403**: Indicates that the user does not have the necessary permissions to edit a field update action. [application/json]
    oneOf:
      - `UpdateFieldUpdateLockedActionPermissionError` — Returned when the target field update is locked and the user lacks privilege to modify it.
        - `code` (string) **REQ** [enum=['NO_PERMISSION']] — Represents the error code.
        - `details` (object) **REQ** — Error details
          - `resource_path_index` (integer/int32) — Index of the resource path used in the request.
        - `message` (string) **REQ** [maxLen=255] — Represents the error message.
        - `status` (string) **REQ** [enum=['error']] — Error status
      - `FeatureNoPermissionError` — Indicates that the user does not have the necessary permissions to create a field update action.
        - `code` (string) **REQ** [enum=['NO_PERMISSION']] — Represents the error code.
        - `details` (object) **REQ** — Error details with validation information
