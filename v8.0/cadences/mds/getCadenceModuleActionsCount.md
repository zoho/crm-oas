...
**Operation:** `getCadenceModuleActionsCount` — Get Cadence module actions count
...
**Parameters:**
- `module` (query, string, required) [maxLen=255]: Specify the CRM module API name to retrieve the Cadence action count for.
...
- **200**: Returns the total count of Cadence actions for the specified module. [application/json]
    > Represents the successful response for the count of Cadence module actions.
    - `count` (integer/int32) — Represents the total count of Cadence actions for the specified module.

- **400**: The request contains invalid data or the required **module** parameter is missing.
**Resolution:** The **module** query parameter must be present and must specify a valid CRM module API name. — Schema: `AllErrorResponse` [application/json]
    > Represents the error response returned when the API request fails due to validation errors or unsupported operations.
