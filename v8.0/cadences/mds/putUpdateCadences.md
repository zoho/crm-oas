...
**Operation:** `putUpdateCadences` — Update multiple Cadences (bulk form)
...
**Request Body** — application/json `CadencesUpdateRequestBody`
> The request body must contain a Cadences array with updated configurations for one or more Cadences.
  > Represents the request body schema for updating the configuration of one or more existing Cadences.
  - `cadences` (array of object) [maxItems=1] **REQ** — Represents the list of Cadences to update.
    - `id` (string) [maxLen=255] — Represents the unique identifier of the Cadence to update.
    - `name` (string) [maxLen=100] — Represents the updated name of the Cadence.
    - `description` (string) [maxLen=500] — Represents the updated description of the Cadence.
    - `execution_details` (object) — Represents the updated execution frequency and unenrollment configuration for the Cadence.
      - `unenroll_properties` (array of object `UnenrollPropertiesNested`) [maxItems=200] — Represents the updated conditions under which records are automatically unenrolled from the Cadence.
...
followup_criteria - Unenroll records based on follow-up criteria.
    - `follow_ups` (array of object `FollowUpsUpdateNested`) [maxItems=100] — Represents the updated list of follow-up steps for the Cadence.
      schema: `FollowUpsUpdateNested`
      - `execute_after` (object `ExecuteAfterNested`) — Cadence followup execute_after
...
months - Execution delay in months.
      - `parent_follow_up` (object) — Represents the parent follow-up action in the Cadence sequence.
        - `reference_id` (string) [maxLen=255] — Represents the reference identifier of the parent follow-up action.
        - `id` (string) [maxLen=255] — Represents the unique identifier of the parent follow-up action.
      - `reference_id` (string) [maxLen=255] — Represents the reference identifier of the follow-up action.
      - `id` (string) [maxLen=255] — Represents the unique identifier of the follow-up action.
      - `action` (object `ActionNested`) — Cadence followup action (Required)
        oneOf:
...
**emailandpopup** — delivers both simultaneously.
      ...
      - `triggers` (array of string) [maxItems=500] — Represents the list of trigger conditions that initiate this follow-up action.
        items: [maxLen=255]

**Responses:**

- **200**: Returns the updated details of the Cadences, confirming that the updates were applied successfully. — Schema: `SuccessResponse200` [application/json]
...
**Resolution:** All field values must be valid, required fields must be present, and the data types must be correct. [application/json]
    > Represents the error response for the bulk Cadence update operation.
    oneOf:
...
- **403**: The user does not have permission to update Cadences.
...
**Scopes:** ZohoCRM.settings.cadences.UPDATE
