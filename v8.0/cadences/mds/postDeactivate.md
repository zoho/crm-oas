...
**Operation:** `postDeactivate` — Deactivate a Cadence
...
**Parameters:**
- `id` (path, string, required) [maxLen=255]: Specify the unique identifier of the Cadence to deactivate.
...
- **200**: Returns a success response confirming the Cadence was deactivated. — Schema: `SuccessResponse200` [application/json]
...
- **400**: The request contains invalid data or the Cadence is already in a deactivated state.
**Resolution:** The Cadence ID must be valid, and the Cadence must currently be in an active state before it can be deactivated. — Schema: `AllErrorResponse` [application/json]
    > Represents the error response returned when the API request fails due to validation errors or unsupported operations.

- **403**: The user does not have permission to deactivate Cadences.
