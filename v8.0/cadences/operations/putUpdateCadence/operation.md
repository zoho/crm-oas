...
**Operation:** `putUpdateCadence` — Cadence
...
> The request body must contain a Cadences array with the updated configuration for the Cadence.
...
- **200**: Returns the updated Cadence details, confirming that the update was applied successfully. — Schema: `SuccessResponse200` [application/json]
...
**Resolution:** All field values must be valid, required fields must be present, and the data types must be correct. [application/json]
    > Represents the error response for the Cadence update operation.
    oneOf:
