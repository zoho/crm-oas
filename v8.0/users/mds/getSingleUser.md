# GET /users/{user}
**Operation:** `getSingleUser` — Get a specific user
...
- `type__s` (query, string, optional) [enum=[7 values], readOnly]: Specify the account type of the users to retrieve.
Possible values:
Sandbox Developer User - User with access to the sandbox environment.
Support User - User with support access.
Client Portal User - External client portal user.
Regular User - Standard CRM user.
Team User - User operating as part of a team account.
System User - User designated as a system-level account.
Digital Employee - Automated or digital worker account.

**Responses:**

- **200**: Returns the details of the specified user. [application/json]
    > Contains the user record returned for the specified user ID.

    - `users` (array of object `GetUserDetails`) [maxItems=200] — Represents the list of user records returned for the specified user ID.
...
- **204**: No content returned. No user record exists for the specified user ID.
