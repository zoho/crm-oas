...
### Status `201` — `application/json` — UpdateShareRecordsSuccess

An example of successful update response.
...
Invalid module API name in the URL
...
### Status `400` — `application/json` — ShareLimitForATypeExceeded

Share limit exceeded for the recipient type
  ...
  "share": [
    {
      ...
      "details": {
        "api_name": "type",
        "limit": 5,
        "json_path": "$.share[5].shared_with.type"
      },
      "message": "Cannot share a record to more than 5 roles",
      "status": "error"
    }
  ]
}
...
### Status `403` — `application/json` — UpdateShareRecordsForbidden
...
An example for record-level sharing unsupported for the module.
