# Examples: getModules

**GET /settings/modules**

## Parameter examples

### `status` (query) — Single

Retrieve only system hidden modules

```json
"system_hidden"
```

### `status` (query) — Multiple

Retrieve visible and user hidden modules

```json
"visible,user_hidden"
```

### `feature_name` (query) — ScoringRules

Retrieve modules for scoring rules feature

```json
"scoring_rules"
```

### `feature_name` (query) — Forecasts

Retrieve modules for forecasts feature

```json
"forecasts"
```

### `include` (query) — CustomView

Include custom view metadata

```json
"custom_view"
```

### `include` (query) — RelatedLists

Include related lists configuration

```json
"related_lists"
```

### `include` (query) — BusinessCardFields

Include business card fields

```json
"business_card_fields"
```

### `include` (query) — Layouts

Include layout configurations

```json
"layouts"
```

### `include` (query) — Fields

Include field metadata

```json
"fields"
```

### `include` (query) — Multiple

Include multiple optional fields

```json
"custom_view,related_lists,business_card_fields,layouts,fields"
```

## Response examples

### Status `200` — `application/json` — AllModulesWithVariations

Complete module list with all variations

```json
{
  "modules": [
    {
      "has_more_profiles": false,
      "access_type": "org_based",
      "private_profile": null,
      "global_search_supported": false,
      "deletable": false,
      "description": null,
      "creatable": false,
      "recycle_bin_on_delete": false,
      "modified_time": null,
      "plural_label": "Home",
      "presence_sub_menu": false,
      "actual_plural_label": "Home",
      "lookupable": false,
      "id": "111111000000002646",
      "isBlueprintSupported": false,
      "visibility": 1,
      "convertable": false,
      "sub_menu_available": false,
      "editable": false,
      "actual_singular_label": "Home",
      "profiles": [
        {
          "name": "Administrator",
          "id": "111111000000000497"
        }
      ],
      "show_as_tab": true,
      "web_link": null,
      "sequence_number": 1,
      "singular_label": "Home",
      "viewable": true,
      "api_supported": false,
      "api_name": "Home",
      "quick_create": false,
      "modified_by": null,
      "generated_type": "default",
      "feeds_required": false,
      "public_fields_configured": false,
      "arguments": [],
      "module_name": "Home",
      "profile_count": 4,
      "business_card_field_limit": 5,
      "parent_module": {},
      "status": "visible"
    },
    {
      "has_more_profiles": false,
      "access_type": "org_based",
      "private_profile": null,
      "global_search_supported": true,
      "deletable": true,
      "description": null,
      "creatable": true,
      "recycle_bin_on_delete": true,
      "modified_time": "2025-11-11T11:29:45+05:30",
      "plural_label": "Leads",
      "presence_sub_menu": true,
      "actual_plural_label": "Leads",
      "lookupable": true,
      "id": "111111000000002648",
      "isBlueprintSupported": true,
      "visibility": 1,
      "convertable": true,
      "sub_menu_available": true,
      "editable": true,
      "actual_singular_label": "Lead",
      "profiles": [
        {
          "name": "Administrator",
          "id": "111111000000000497"
        }
      ],
      "show_as_tab": true,
      "web_link": null,
      "sequence_number": 2,
      "singular_label": "Lead",
      "viewable": true,
      "api_supported": true,
      "api_name": "Leads",
      "quick_create": true,
      "modified_by": {
        "name": "Rajaa",
        "id": "111111000000057804"
      },
      "generated_type": "default",
      "feeds_required": false,
      "public_fields_configured": true,
      "arguments": [],
      "module_name": "Leads",
      "profile_count": 4,
      "business_card_field_limit": 5,
      "parent_module": {},
      "status": "visible"
    },
    {
      "has_more_profiles": false,
      "access_type": "org_based",
      "private_profile": null,
      "global_search_supported": true,
      "deletable": true,
      "description": null,
      "creatable": true,
      "recycle_bin_on_delete": true,
      "modified_time": "2025-11-21T15:10:43+05:30",
      "plural_label": "Subform 2",
      "presence_sub_menu": false,
      "actual_plural_label": "Subform 2",
      "lookupable": false,
      "id": "111111000000270418",
      "isBlueprintSupported": false,
      "visibility": 2,
      "convertable": false,
      "sub_menu_available": false,
      "editable": true,
      "static_subform_properties": {
        "fields": [
          {
            "api_name": "Pick_List_1",
            "id": "111111000000271161"
          }
        ]
      },
      "actual_singular_label": "Subform 2",
      "profiles": [
        {
          "name": "Administrator",
          "id": "111111000000000497"
        }
      ],
      "show_as_tab": false,
      "web_link": null,
      "sequence_number": 25,
      "singular_label": "Subform 2",
      "viewable": true,
      "api_supported": true,
      "api_name": "Subform_2",
      "quick_create": false,
      "modified_by": {
        "name": "Rajaa",
        "id": "111111000000057804"
      },
      "generated_type": "static_subform",
      "feeds_required": false,
      "public_fields_configured": false,
      "arguments": [],
      "module_name": "LinkingModule2",
      "profile_count": 4,
      "business_card_field_limit": 5,
      "parent_module": {
        "api_name": "CM",
        "id": "111111000000257585"
      },
      "status": "visible"
    },
    {
      "has_more_profiles": false,
      "access_type": "org_based",
      "private_profile": null,
      "global_search_supported": false,
      "deletable": false,
      "description": null,
      "creatable": false,
      "recycle_bin_on_delete": false,
      "modified_time": "2025-11-20T02:10:35+05:30",
      "plural_label": "webtab",
      "presence_sub_menu": false,
      "actual_plural_label": "webtab",
      "lookupable": false,
      "id": "111111000000265237",
      "isBlueprintSupported": false,
      "visibility": 1,
      "convertable": false,
      "sub_menu_available": false,
      "editable": false,
      "actual_singular_label": "webtab",
      "profiles": [
        {
          "name": "Administrator",
          "id": "111111000000000497"
        }
      ],
      "show_as_tab": true,
      "web_link": "https://www.youtube.com/watch?v=-pE5fXPHbdo##wtab##?id=${Users.recordId}&name=${Users.111111000000000107}&id=${Users.recordId}",
      "sequence_number": 122,
      "singular_label": "webtab",
      "viewable": true,
      "api_supported": false,
      "api_name": "webtab",
      "quick_create": false,
      "modified_by": {
        "name": "Rajaa",
        "id": "111111000000057804"
      },
      "generated_type": "webtab_link",
      "feeds_required": false,
      "public_fields_configured": false,
      "arguments": [
        {
          "name": "id",
          "value": "users.recordId"
        },
        {
          "name": "name",
          "value": "users.first_name"
        },
        {
          "name": "id",
          "value": "users.recordId"
        }
      ],
      "module_name": "WebTab1",
      "profile_count": 1,
      "business_card_field_limit": 5,
      "parent_module": {},
      "status": "visible"
    },
    {
      "has_more_profiles": false,
      "access_type": "team_based",
      "private_profile": {
        "name": "Admins",
        "id": "111111000000000501"
      },
      "global_search_supported": true,
      "deletable": true,
      "description": null,
      "creatable": true,
      "recycle_bin_on_delete": true,
      "modified_time": "2025-11-21T12:12:26+05:30",
      "plural_label": "sdfvg",
      "presence_sub_menu": true,
      "actual_plural_label": "sdfvg",
      "lookupable": true,
      "id": "111111000000268896",
      "isBlueprintSupported": true,
      "visibility": 1,
      "convertable": false,
      "sub_menu_available": true,
      "editable": true,
      "actual_singular_label": "sdfvgdfvsg",
      "profiles": [
        {
          "name": "Admins",
          "id": "111111000000000501"
        }
      ],
      "show_as_tab": true,
      "web_link": null,
      "sequence_number": 125,
      "singular_label": "sdfvgdfvsg",
      "viewable": true,
      "api_supported": true,
      "api_name": "sdfvg",
      "quick_create": true,
      "modified_by": {
        "name": "Rajaa",
        "id": "111111000000057804"
      },
      "generated_type": "custom",
      "feeds_required": false,
      "public_fields_configured": false,
      "arguments": [],
      "module_name": "CustomModule3",
      "profile_count": 5,
      "business_card_field_limit": 5,
      "parent_module": {},
      "status": "visible"
    },
    {
      "has_more_profiles": false,
      "access_type": "org_based",
      "private_profile": null,
      "global_search_supported": true,
      "deletable": true,
      "description": null,
      "creatable": true,
      "recycle_bin_on_delete": true,
      "modified_time": "2025-11-20T15:50:22+05:30",
      "plural_label": "Custom Modules",
      "presence_sub_menu": true,
      "actual_plural_label": "Custom Modules",
      "lookupable": true,
      "id": "111111000000242167",
      "isBlueprintSupported": true,
      "visibility": 1,
      "convertable": false,
      "sub_menu_available": true,
      "editable": true,
      "actual_singular_label": "Custom Module",
      "profiles": [
        {
          "name": "Administrator",
          "id": "111111000000000497"
        },
        {
          "name": "Standard",
          "id": "111111000000000499"
        },
        {
          "name": "Team User",
          "id": "111111000000263146"
        }
      ],
      "show_as_tab": true,
      "web_link": null,
      "sequence_number": 115,
      "singular_label": "Custom Module",
      "viewable": true,
      "api_supported": true,
      "api_name": "CM1",
      "quick_create": true,
      "modified_by": {
        "name": "Rajaa",
        "id": "111111000000057804"
      },
      "generated_type": "custom",
      "feeds_required": false,
      "public_fields_configured": false,
      "arguments": [],
      "module_name": "CustomModule2",
      "profile_count": 3,
      "business_card_field_limit": 5,
      "parent_module": {},
      "status": "visible"
    },
    {
      "has_more_profiles": false,
      "access_type": "org_based",
      "private_profile": null,
      "global_search_supported": true,
      "deletable": true,
      "description": null,
      "creatable": true,
      "recycle_bin_on_delete": true,
      "modified_time": null,
      "plural_label": "Notes",
      "presence_sub_menu": false,
      "actual_plural_label": "Notes",
      "lookupable": false,
      "id": "111111000000002676",
      "isBlueprintSupported": false,
      "visibility": 1,
      "convertable": false,
      "sub_menu_available": false,
      "editable": true,
      "actual_singular_label": "Note",
      "profiles": [
        {
          "name": "Administrator",
          "id": "111111000000000497"
        }
      ],
      "show_as_tab": false,
      "web_link": null,
      "sequence_number": 32,
      "singular_label": "Note",
      "viewable": true,
      "api_supported": true,
      "api_name": "Notes",
      "quick_create": false,
      "modified_by": null,
      "generated_type": "default",
      "feeds_required": false,
      "public_fields_configured": false,
      "arguments": [],
      "module_name": "Notes",
      "profile_count": 4,
      "business_card_field_limit": 5,
      "parent_module": {},
      "status": "system_hidden"
    },
    {
      "has_more_profiles": false,
      "access_type": "org_based",
      "private_profile": null,
      "global_search_supported": false,
      "deletable": false,
      "description": null,
      "creatable": false,
      "recycle_bin_on_delete": false,
      "modified_time": null,
      "plural_label": "Feeds",
      "presence_sub_menu": false,
      "actual_plural_label": "Feeds",
      "lookupable": false,
      "id": "111111000000002662",
      "isBlueprintSupported": false,
      "visibility": 1,
      "convertable": false,
      "sub_menu_available": false,
      "editable": false,
      "actual_singular_label": "Feeds",
      "profiles": [
        {
          "name": "Administrator",
          "id": "111111000000000497"
        }
      ],
      "show_as_tab": true,
      "web_link": null,
      "sequence_number": 15,
      "singular_label": "Feeds",
      "viewable": true,
      "api_supported": false,
      "api_name": "Feeds",
      "quick_create": false,
      "modified_by": null,
      "generated_type": "default",
      "feeds_required": false,
      "public_fields_configured": false,
      "arguments": [],
      "module_name": "Feeds",
      "profile_count": 4,
      "business_card_field_limit": 5,
      "parent_module": {},
      "status": "user_hidden"
    },
    {
      "has_more_profiles": false,
      "access_type": "org_based",
      "private_profile": null,
      "global_search_supported": false,
      "deletable": false,
      "description": null,
      "creatable": false,
      "recycle_bin_on_delete": false,
      "modified_time": null,
      "track_current_data": true,
      "plural_label": "Stage History",
      "presence_sub_menu": false,
      "actual_plural_label": "Stage History",
      "lookupable": false,
      "id": "111111000000002792",
      "isBlueprintSupported": false,
      "visibility": 2,
      "convertable": false,
      "sub_menu_available": false,
      "editable": false,
      "actual_singular_label": "Stage History",
      "profiles": [
        {
          "name": "Standard",
          "id": "111111000000000499"
        }
      ],
      "show_as_tab": false,
      "web_link": null,
      "sequence_number": 38,
      "singular_label": "Stage History",
      "viewable": false,
      "api_supported": true,
      "api_name": "DealHistory",
      "quick_create": false,
      "modified_by": null,
      "generated_type": "field_tracker",
      "feeds_required": false,
      "public_fields_configured": false,
      "arguments": [],
      "module_name": "DealHistory",
      "profile_count": 2,
      "business_card_field_limit": 5,
      "parent_module": {
        "api_name": "Deals",
        "id": "111111000000002654"
      },
      "status": "system_hidden"
    },
    {
      "has_more_profiles": false,
      "access_type": "org_based",
      "private_profile": null,
      "global_search_supported": false,
      "deletable": true,
      "description": null,
      "creatable": false,
      "recycle_bin_on_delete": true,
      "modified_time": null,
      "plural_label": "Calendar Bookings",
      "presence_sub_menu": false,
      "actual_plural_label": "Calendar Bookings",
      "lookupable": false,
      "id": "111111000000002824",
      "isBlueprintSupported": false,
      "visibility": 14316,
      "convertable": false,
      "sub_menu_available": false,
      "editable": false,
      "actual_singular_label": "Calendar Booking",
      "profiles": [
        {
          "name": "Administrator",
          "id": "111111000000000497"
        }
      ],
      "show_as_tab": false,
      "web_link": null,
      "sequence_number": 53,
      "singular_label": "Calendar Booking",
      "viewable": false,
      "api_supported": false,
      "api_name": "CalendarBookings__s",
      "quick_create": false,
      "modified_by": null,
      "generated_type": "default",
      "feeds_required": false,
      "public_fields_configured": false,
      "arguments": [],
      "module_name": "CalendarBookings",
      "profile_count": 3,
      "business_card_field_limit": 5,
      "parent_module": {},
      "status": "visible"
    },
    {
      "has_more_profiles": false,
      "access_type": "org_based",
      "private_profile": null,
      "global_search_supported": true,
      "deletable": false,
      "description": null,
      "creatable": false,
      "recycle_bin_on_delete": false,
      "modified_time": "2025-11-10T16:28:58+05:30",
      "plural_label": "File Upload 1",
      "presence_sub_menu": false,
      "actual_plural_label": "File Upload 1",
      "lookupable": false,
      "id": "111111000000225531",
      "isBlueprintSupported": false,
      "visibility": 2046,
      "convertable": false,
      "sub_menu_available": true,
      "editable": false,
      "actual_singular_label": "File Upload 1",
      "profiles": [],
      "show_as_tab": true,
      "web_link": null,
      "sequence_number": 111,
      "singular_label": "File Upload 1",
      "viewable": false,
      "api_supported": false,
      "api_name": "File_Upload_1__s",
      "quick_create": false,
      "modified_by": {
        "name": "Rajaa",
        "id": "111111000000057804"
      },
      "generated_type": "default",
      "feeds_required": false,
      "public_fields_configured": false,
      "arguments": [],
      "module_name": "Fileupload1",
      "profile_count": 0,
      "business_card_field_limit": 5,
      "parent_module": {},
      "status": "visible"
    }
  ]
}
```

### Status `400` — `application/json` — InvalidStatus

Invalid status parameter value

```json
{
  "code": "INVALID_DATA",
  "details": {
    "index": 0,
    "supported_values": [
      "visible",
      "user_hidden",
      "system_hidden",
      "scheduled_for_deletion"
    ],
    "param_name": "status"
  },
  "message": "The status given is invalid.",
  "status": "error"
}
```

### Status `400` — `application/json` — InvalidFeatureName

Empty or invalid feature_name parameter

```json
{
  "code": "INVALID_DATA",
  "details": {},
  "message": "the given feature name is invalid",
  "status": "error"
}
```
