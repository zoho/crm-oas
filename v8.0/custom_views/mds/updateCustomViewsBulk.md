...
**Operation:** `updateCustomViewsBulk` — Custom Views
...
`InvalidDataType`:
  > Represents the error response returned when a criterion field value is provided with an incorrect data type. Contains the error code, a descriptive message, the response status, and details identifying the offending field and its expected data type.
...
> The request body must contain a custom_views array with one or more objects, each specifying the ID and updated configuration for a Custom View.
  > Represents the request body envelope for bulk updating one or more Custom Views.
  - `custom_views` (array of object `CustomViewPutBulkItem`) [maxItems=200] — Specify the list of Custom Views to update in bulk. Each object must include the Custom View ID and the fields to update.
    schema: `CustomViewPutBulkItem`
    - `id` (string/int64) **REQ** [maxLen=25] — Represents the unique ID of the Custom View to update.
    - `name` (string) [maxLen=50] — Represents the name of the Custom View.
    - `access_type` (string) [enum=['shared', 'public', 'only_to_me']] — Represents the access type for the Custom View.
Possible values:
...
- **200**: Returns a success response indicating that all specified Custom Views were updated successfully. — Schema: `CustomViewUpdateSuccessResponse` [application/json]
...
- **207**: Returns a multi-status response when the bulk update partially succeeded. Each item indicates whether the update succeeded or failed for that Custom View. [application/json]
    > Represents the multi-status response for the bulk update operation, containing a mix of success and failure items.
    - `custom_views` (array of object) [minItems=2, maxItems=200] — Represents the list of bulk update results, including both successful and failed items.
      oneOf:
        ...
        - `CustomViewWrapFailure` — Represents the error response containing Custom View wrap failure details.
          - `custom_views` (array of object `CustomViewWrapFailureItem`) [maxItems=200] **REQ** — Represents the list of Custom View wrap failure items.
        - `CustomViewUpdateFailureItem` — Represents the update failure details for a Custom View, containing the error code, status, message, and additional context.

- **400**: The request contains invalid data or a required field is missing. Resolution: Verify that all required fields are present and contain valid values, and that each object in the array includes a valid Custom View ID. [application/json]
    > Represents the error response for an invalid bulk update request.
    oneOf:
...
**INVALID_MODULE** - The specified module does not exist or is invalid.
      ...
      - `CustomViewUpdateFailureDetails` — Wrapped error: per-item update failures inside custom_views array
        - `custom_views` (array of object `CustomViewUpdateFailureItem`) [maxItems=200] **REQ** — Represents the list of Custom View update failure items.
...
- **401**: Authentication failed or the OAuth access token does not include the required scope. Resolution: A new access token must be generated with the ZohoCRM.settings.custom_views.UPDATE scope. [application/json]
    > Represents the error response for an unauthorized bulk update request.
    oneOf:
...
- **403**: Permission denied to update Custom Views for this module. Resolution: The CRM administrator must grant the required permission to the user's profile. [application/json]
    > Represents the error response for a forbidden bulk update request.
    oneOf:
