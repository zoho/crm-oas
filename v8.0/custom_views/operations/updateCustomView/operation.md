...
**Operation:** `updateCustomView` — CustomView update
...
> The request body must contain a custom_views array with one object specifying the updated configuration for the Custom View.
  > Represents the request body envelope for updating a specific Custom View by its ID.
  - `custom_views` (array of object `CustomViewPutByIdItem`) [maxItems=1] **REQ** — Specify the list of Custom View configurations to update.
    schema: `CustomViewPutByIdItem`
...
- **200**: Returns a success response indicating that the Custom View update completed successfully. — Schema: `CustomViewUpdateSuccessResponse` [application/json]
...
- **400**: The request contains invalid data or a required field is missing. Resolution: Verify that all required fields are present and contain valid values. [application/json]
    > Represents the error response for an invalid update Custom View request.
    oneOf:
...
- **401**: Authentication failed or the OAuth access token does not include the required scope. Resolution: A new access token must be generated with the ZohoCRM.settings.custom_views.UPDATE scope. [application/json]
    > Represents the error response for an unauthorized update Custom View request.
    oneOf:
...
- **403**: Permission denied to update the specified Custom View. Resolution: The CRM administrator must grant the required permission to the user's profile. [application/json]
    > Represents the error response for a forbidden update Custom View request.
    oneOf:
...
**error** - The request encountered a permission error.
        - `details` (object) **REQ** — Represents the additional details about the permission error, including the required permissions.
          - `permissions` (array of string) [maxItems=50] **REQ** — Represents the list of permissions required to perform the action on this module.
            items: [maxLen=255]
      - `ForbiddenByIdErrorResponse` — Represents the error response returned when the user does not have permission to access or modify a specific resource identified by its path.
        - `code` (string) **REQ** [maxLen=50, enum=['NO_PERMISSION']] — Represents the error code for the permission denial.
Possible values:
**NO_PERMISSION** - The user does not have the required permission to perform the operation.
        - `details` (object) **REQ** — Represents the additional details about the permission error.
          - `resource_path_index` (integer/int32) **REQ** — Index of the resource path segment that caused the permission error
        - `message` (string) **REQ** [maxLen=200] — Represents the error message describing the permission denial.
        - `status` (string) **REQ** [enum=['error']] — Represents the status of the response.
Possible values:
**error** - The request encountered a permission error.
