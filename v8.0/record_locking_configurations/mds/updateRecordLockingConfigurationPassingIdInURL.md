...
**Operation:** `updateRecordLockingConfigurationPassingIdInURL` — Update specific record locking configuration
...
**Parameters:**
- `id` (path, string/int64, required): Unique identifier of the record locking configuration. This ID is returned in the response when a configuration is created or retrieved.
- `module` (query, string, required) [maxLen=256]: API name of the module. Use the [Get Modules API](modules.yaml#$.paths./settings/modules.get) to retrieve module names.

**Request Body** (required) — application/json
  > Request body for updating a record locking configuration.
  - `record_locking_configurations` (array of object) [maxItems=1] **REQ** — Array containing the updated configuration.
    ...
    - `locked_for` (string) [enum=['all_profiles', 'all_profiles_except_excluded_profiles']] — Indicates which profiles the lock applies to.
    ...
    - `excluded_fields` (array of object) [maxItems=15] — Fields that are exempt from locking.
      - `id` (string/int64) **REQ** — Field ID. Use the [Get Fields Metadata API](fields.yaml#$.paths./settings/fields.get) to retrieve field IDs.
    ...
    - `id` (string) **REQ** [maxLen=32] — Unique identifier of the configuration to update. This field is required.
    - `lock_excluded_profiles` (array of object) [maxItems=15] — Profiles that are excluded from locking. Required when `locked_for` is `all_profiles_except_excluded_profiles`.
...
- **200**: Successfully updated the record locking configuration. [application/json]
    ...
    - `record_locking_configurations` (array of object) [maxItems=1] **REQ** — Array containing the update response.
      - `status` (string) **REQ** [maxLen=256, enum=['success']] — Indicates successful update.
      ...
      - `details` (object) **REQ** — Additional details about the updated configuration.
        - `id` (string) **REQ** [maxLen=256] — ID of the updated configuration.
