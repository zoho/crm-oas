...
**Operation:** `getWorkflowRules` — List Workflow Rules in your Zoho CRM Organization
...
**Parameters:**
- `page` (query, integer/int32, optional) [min=1]: Specifies the page number for paginated results. Page numbering starts at 1.
- `per_page` (query, integer/int32, optional) [min=1, max=200]: Specifies the number of workflow rules to return per page. The default and maximum value is 200.
...
- `status_active` (query, boolean, optional): Filters workflow rules by activation status. Set to true to return only active rules, or false to return only inactive rules.
- `execute_on` (query, string, optional) [maxLen=25]: Specifies the trigger type to filter workflow rules by, such as create, edit, or delete events.
- `sort_order` (query, string, optional) [enum=['asc', 'desc']]: Specifies the sort direction for the workflow rule list. Accepted values are `asc` and `desc`.
- `sort_by` (query, string, optional) [enum=['modified_time']]: Specifies the field to sort workflow rules by. Only `modified_time` is supported.
- `include_inner_details` (query, boolean, optional): When set to true, includes full condition details, criteria, and action configurations in the response. Defaults to false.
- `filter` (query, string, optional) [maxLen=1000]: Specifies a JSON filter expression to search workflow rules by name. Supports only contains-type matching on the rule name field.
...
- **200**: Returns a paginated list of workflow rules defined in the Zoho CRM organization. — Schema: `WorkflowRulesGetResponseSchema` [application/json]
    ...
    schema: `WorkflowRulesGetResponseSchema`
    - `workflow_rules` (array of object `WorkflowRuleSingleGetSchema`) [maxItems=200] **REQ** — Represents the array of workflow rule objects returned in the list response.
...
false - The workflow rule is inactive and does not execute, even when trigger conditions are met.
    - `info` (object `InfoNestedSchema`) **REQ** — Represents the pagination metadata included in list response payloads, including the current page, records per page, total count, and whether more records exist.
      schema: `InfoNestedSchema`
      - `per_page` (integer/int32) **REQ** — Represents the number of records returned per page.
      - `count` (integer/int32) **REQ** — Represents the total number of records matching the query.
      - `page` (integer/int32) **REQ** — Represents the current page number in the paginated response.
      - `more_records` (boolean) **REQ** — Indicates whether more records exist beyond the current page.

- **400**: The request contains an invalid parameter value.
**Resolution:** The module API name and all other parameter values must be valid; the details object identifies the offending parameter. — Schema: `InvalidDataNoApiNameAndPathSchema` [application/json]
...
- **403**: The workflow rules feature is not available in the current CRM edition.
**Resolution:** The CRM account must be upgraded to a Zoho CRM edition that includes the workflow rules feature. — Schema: `NoPermissionSchema` [application/json]
