...
**Operation:** `updateWorkflowRuleById` — Update Workflow Rule by ID
...
**Request Body** — application/json `UpdateWorkflowRuleSchema`
> The request body must contain a workflow_rules array with exactly one object representing the updated workflow rule configuration.
  ...
  - `workflow_rules` (array of object `WorkflowRulePutNestedSchema`) [maxItems=1] **REQ** — The request body must contain a **workflow_rules** array with one object.
    schema: `WorkflowRulePutNestedSchema`
    - `id` (string) [maxLen=30] — Represents the unique ID of the workflow rule to update. Required.
    - `status` (object) — Specify the activation state of the workflow rule. When setting **active** to false, you can optionally delete all associated scheduled actions.
      - `active` (boolean) — Specify whether the workflow rule is active or inactive.
Possible values:
true - The rule is active and executes when triggered.
false - The rule is inactive and does not execute.
      - `delete_schedule_action` (boolean) — Specify whether to delete all scheduled actions associated with the workflow rule when deactivating it.
Possible values:
true - Delete all scheduled actions when the rule is deactivated.
false - Preserve scheduled actions when the rule is deactivated.
    - `execute_when` (object `ExecuteWhenNestedSchema`) — Defines the conditions under which the workflow rule will be executed. (Required)
      discriminator: `type`
...
hours - Window measured in hours.
            ...
            - `trigger_module` (object `ModuleOrFieldNestedSchema`) — The module that actually triggers the rule. Defaults to the rule's top-level module if omitted. Must be explicitly set when the trigger targets a different module  - e.g., a rule on Leads triggered by 'note created' requires trigger_module = {api_name: 'Notes', id: '...'}. Use `getModules` to discover module api_name and id values.
    - `name` (string) [maxLen=100] — Specify the workflow rule name. Required.
    - `description` (string) [maxLen=1000] — Specify the workflow rule description.
    - `conditions` (array of object `ConditionsNestedSchema`) [maxItems=20] — Specify the array of condition objects to update. On update, unspecified conditions are preserved. Conditions with a null ID are added; conditions with an existing ID get replaced; conditions with a **_delete** flag are removed.
    ...
    - `lock` (object) — Specify whether the workflow rule should be locked to prevent editing by other users.
      - `message` (string/string) — Specify a note or label indicating the reason or context for locking the rule.
      - `status` (boolean) [enum=[True, False]] — Specify whether to lock or unlock the workflow rule.
Possible values:
true - Locks the rule to prevent editing.
false - Unlocks the rule.
    - `module` (object `ModuleOrFieldNestedSchema`) — Represents a CRM module or field reference using its API name and unique numeric ID.

**Responses:**

- **200**: Returns the result of the workflow rule update, including the unique ID of the updated rule.
 — Schema: `WorkflowSuccessSchema` [application/json]
...
- **400**: The request contains invalid or conflicting data.
**Resolution:** Review the error details in the response body and correct the request. Common causes include invalid field values, duplicate actions, missing dependent fields, ambiguous criteria, or exceeded rule and action limits. [application/json]
    oneOf:
      - `UrlIdMissingSchema` — Represents the error response returned when the workflow rule ID provided in the URL path is invalid or does not match an existing record.
        ...
        - `workflow_rules` (array of object) [maxItems=25] **REQ** — Represents the array of workflow rule error objects returned when the request body fails validation.

          oneOf:
            - `DependentFieldMissingSchema` — Error code DEPENDENT_FIELD_MISSING occurs when: action details are missing or empty, assign_owner resource is omitted, scheduled_actions execute_after lacks period or unit, or field_update lookup_field is not provided. To fix, check the action type's required companion fields and include all of them.
              - `code` (string) **REQ** [enum=['DEPENDENT_FIELD_MISSING']] — Represents the error code that identifies the specific error condition.
              - `details` (object) **REQ** — Represents the error details containing additional context about the failed request.
                - `dependee` (object) — Represents the **dependee** value from the error detail object.
                  - `api_name` (string) [maxLen=255] — Represents the **api_name** value from the error detail object.
                  - `json_path` (string) [maxLen=1000] — Represents the **json_path** value from the error detail object.
                - `api_name` (string) [maxLen=255] — Represents the **api_name** value from the error detail object.
                - `json_path` (string) [maxLen=1000] — Represents the **json_path** value from the error detail object.
              - `message` (string) **REQ** [maxLen=1000] — Represents the error message describing the issue.
              - `status` (string) **REQ** [enum=['error']] — Represents the response status for the error, typically **error**.
            - `AlreadyUsedSchema` — Represents the error response returned when the resource or value you are trying to create already exists in the system.
              - `code` (string) **REQ** [enum=['ALREADY_USED']] — Represents the error code returned when a resource or value is already in use and cannot be reused in the requested operation.
              - `message` (string) **REQ** [maxLen=1000] — Represents the error message describing the issue.
              - `status` (string) **REQ** [enum=['error']] — Represents the response status for the error, typically **error**.
              - `details` (object) **REQ** — Represents the error details containing additional context about the failed request.
                - `api_name` (string) **REQ** [maxLen=50] — Represents the **api_name** value from the error detail object.
                - `json_path` (string) **REQ** [maxLen=1000] — Represents the **json_path** value from the error detail object.
                - `exists_in` (object) — Represents the **exists_in** value from the error detail object.
                  - `json_path` (string) **REQ** [maxLen=1000] — Represents the **json_path** value from the error detail object.
                  - `api_name` (string) **REQ** [maxLen=50] — Represents the **api_name** value from the error detail object.
            - `ExpectedFieldMissingSchema` — Represents the error response returned when required fields such as the module API name and ID are missing from the request.
              - `code` (string) **REQ** [enum=['EXPECTED_FIELD_MISSING']] — Represents the error code that identifies the specific error condition.
              - `details` (object) **REQ** — Represents the error details containing additional context about the failed request.
                - `expected_fields` (array of object) [maxItems=25] — Represents the **expected_fields** value from the error detail object.
                  - `api_name` (string) [maxLen=255] — Represents the **api_name** value from the error detail object.
                  - `json_path` (string) [maxLen=1000] — Represents the **json_path** value from the error detail object.
              - `message` (string) **REQ** [maxLen=1000] — Represents the error message describing the issue.
              - `status` (string) **REQ** [enum=['error']] — Represents the response status for the error, typically **error**.
            - `AmbiguityDuringProcessingSchema` — Represents the error response returned when the system encounters ambiguity during processing due to conflicting field references.
              - `code` (string) **REQ** [enum=['AMBIGUITY_DURING_PROCESSING']] — Represents the error code that identifies the specific error condition.
              - `details` (object) **REQ** — Represents the error details containing additional context about the failed request.
                - `ambiguity_due_to` (array of object) [maxItems=2] — Represents the **ambiguity_due_to** value from the error detail object.
                  - `api_name` (string) **REQ** [maxLen=255] — Represents the API name of the conflicting field that caused the ambiguity.
                  - `json_path` (string) **REQ** [maxLen=1000] — Represents the JSON path of the conflicting field that caused the ambiguity.
              - `message` (string) **REQ** [maxLen=1000] — Represents the error message describing the issue.
              - `status` (string) **REQ** [enum=['error']] — Represents the response status for the error, typically **error**.
            - `LimitExceededSchema` — Represents the error response returned when a configured limit, such as the maximum number of conditions or actions per rule, is exceeded.
              - `code` (string) **REQ** [enum=['LOOKUP_LIMIT_EXCEEDED', 'LIMIT_EXCEEDED']] — Represents the error code that identifies the specific error condition.
              - `message` (string) **REQ** [maxLen=1000] — Represents the error message describing the issue.
              - `status` (string) **REQ** [enum=['error']] — Represents the response status for the error, typically **error**.
              - `details` (object) **REQ** — Represents the error details containing additional context about the failed request.
                - `limit` (integer/int32) — Represents the **limit** value from the error detail object.
                - `api_name` (string) [maxLen=255] — Represents the **api_name** value from the error detail object.
                - `json_path` (string) [maxLen=1000] — Represents the **json_path** value from the error detail object.
                - `lookupFields` (array of string) [maxItems=50] — Represents the **lookupFields** value from the error detail object.
                  items: [maxLen=255]
            - `CannotRemoveSchema` — Represents the error response returned when a delete operation would leave required data empty or remove a resource that is still in use.
              - `code` (string) **REQ** [enum=['CANNOT_REMOVE']] — Represents the error code that identifies the specific error condition.
              - `message` (string) **REQ** [maxLen=1000] — Represents the error message describing the issue.
              - `status` (string) **REQ** [enum=['error']] — Represents the response status for the error, typically **error**.
              - `details` (object) **REQ** — Represents the error details identifying the resource that cannot be removed from the workflow rule.
                - `api_name` (string) [maxLen=255] — Represents the API name of the resource that cannot be removed.
                - `json_path` (string) [maxLen=1000] — Represents the JSON path to the resource that cannot be removed.
            - `RequiredDataNotFoundSchema` — Represents the error response returned when required data referenced in the request, such as a module, field, or action entity, cannot be found in the system.
              - `code` (string) **REQ** [enum=['REQUIRED_DATA_NOT_FOUND']] — Represents the error code that identifies the specific error condition.
              - `message` (string) **REQ** [maxLen=1000] — Represents the error message describing the issue.
              - `status` (string) **REQ** [enum=['error']] — Represents the response status for the error, typically **error**.
              - `details` (object) **REQ** — Represents the error details containing additional context about the failed request.
                - `api_name` (string) [maxLen=255] — Represents the API name of the field with missing required data.
                - `json_path` (string) [maxLen=1000] — Represents the JSON path to the field with missing required data.
                - `value` (string) [maxLen=255] — Represents the specific required value that was not found in the request.
            - `FailureSchema` — Represents a generic failure response returned when an unexpected error occurs during workflow rule creation or update.
              - `code` (string) **REQ** [enum=['FAILURE']] — Represents the error code that identifies the specific error condition.
              - `message` (string) **REQ** [maxLen=1000] — Represents the error message describing the issue.
              - `status` (string) **REQ** [enum=['error']] — Represents the response status for the error, typically **error**.
              - `details` (object) **REQ** — Represents the error details providing additional context about the failure.
                - `message` (string) [maxLen=1000] — Represents the internal error message describing the failure.

- **403**: Permission denied for the requested resource.
**Resolution:** The authenticated user does not have permission to perform this action. Only users with the Manage Workflow permission in their CRM profile can update workflow rules. — Schema: `NoPermissionSchema` [application/json]
