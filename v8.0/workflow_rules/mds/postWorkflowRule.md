...
**Operation:** `postWorkflowRule` — Create a workflow rule for a module
...
**Request Body** — application/json `PostWorkflowRuleSchema`
> Workflow rule configuration payload.
  ...
  - `workflow_rules` (array of object `WorkflowRulePostNestedSchema`) [maxItems=1] **REQ** — Represents the array containing the workflow rule object to create. The array must contain exactly one object per request.
    schema: `WorkflowRulePostNestedSchema`
    - `name` (string) **REQ** [maxLen=100] — Specify the workflow rule name. Required. Cannot exceed 100 characters. Special characters are not allowed.
    - `description` (string) [maxLen=1000, nullable] — Specify an optional description for the workflow rule. Set to null to clear an existing description.
    - `module` (object `ModuleOrFieldNestedSchema`) **REQ** — The module to which the workflow rule applies. Use the [Get Modules API](modules.yaml#$.paths./settings/modules.get) to retrieve module IDs. Required. Must be a workflow-supported module. If both id and api_name are provided, they must match. Use `getWorkflowConfigurations` to discover supported modules. Cannot be changed after creation.
    - `execute_when` (object `ExecuteWhenNestedSchema`) **REQ** — Specifies when the workflow rule is triggered. This is a required JSON object with "type" and "details" properties. For example: {"type": "create_or_edit", "details": {"trigger_module": {"api_name": "Leads", "id": "..."}, "repeat": false}}. Passing a string instead of a JSON object, such as "create_or_edit", returns an "INVALID_DATA" error. The "details.trigger_module" must match the module associated with the workflow rule. Use "getWorkflowConfigurations" to retrieve the available triggers and their supported actions.
      discriminator: `type`
...
hours - Window measured in hours.
            ...
            - `trigger_module` (object `ModuleOrFieldNestedSchema`) — The module that actually triggers the rule. Defaults to the rule's top-level module if omitted. Must be explicitly set when the trigger targets a different module  - e.g., a rule on Leads triggered by 'note created' requires trigger_module = {api_name: 'Notes', id: '...'}. Use `getModules` to discover module api_name and id values.
    - `conditions` (array of object `ConditionsNestedSchema`) [maxItems=10] **REQ** — Specify the conditions that define the branching logic of the workflow rule. Each condition evaluates records against criteria and executes its own set of actions. At least one condition is required.
    ...
    - `lock` (object) — Specify the lock configuration to prevent other users from editing the rule. Only administrators can lock or unlock rules.
      - `message` (string/string) — Specify the reason or context for locking the rule. Valid only when **lock.status** is set to true.
      - `status` (boolean) — Specify whether to lock the rule.
Possible values:
...
- **201**: Workflow Rule created successfully — Schema: `WorkflowSuccessSchema` [application/json]
...
- **400**: The response returned when the request contains invalid, missing, or unsupported data. Resolution: review the error details to identify the field or value that failed validation, correct it, and resubmit the request
 [application/json]
    oneOf:
        - `workflow_rules` (array of object) [maxItems=25] **REQ** — Array of error objects
          oneOf:
            ...
            - `LimitExceededSchema` — Represents the error response returned when a configured limit, such as the maximum number of conditions or actions per rule, is exceeded.
              ...
              - `details` (object) **REQ** — Represents the error details containing additional context about the failed request.
                ...
                - `lookupFields` (array of string) [maxItems=50] — Represents the **lookupFields** value from the error detail object.
                  items: [maxLen=255]
            - `DependentMisMatchSchema` — Represents the error response returned when **module_selection** is set to a value that requires a specific module but no module is provided.
              - `code` (string) **REQ** [enum=['DEPENDENT_MISMATCH']] — Represents the error code that identifies the specific error condition.
              - `details` (object) **REQ** — Represents the error details containing additional context about the failed request.
                - `dependee` (object) — Represents the **dependee** value from the error detail object.
                  - `api_name` (string) [maxLen=255] — Represents the **api_name** value from the error detail object.
                  - `json_path` (string) [maxLen=1000] — Represents the **json_path** value from the error detail object.
            ...
            - `FeatureDisabledSchema` — Represents the error response returned when a feature required by the workflow rule configuration is currently disabled in the organization.
              - `code` (string) **REQ** [enum=['FEATURE_DISABLED']] — Represents the error code that identifies the specific error condition.
              - `message` (string) **REQ** [maxLen=1000] — Represents the error message describing the issue.
              - `status` (string) **REQ** [enum=['error']] — Represents the response status for the error, typically **error**.
              - `details` (object) **REQ** — Represents the error details identifying the reason the feature is disabled.
                - `message` (string) [maxLen=1000] — Represents the reason the feature is disabled.
