...
**Operation:** `associateContactRoleToDeal` — Deal Contact Roles
...
**Request Body** (required) — application/json
  ...
  - `data` (array of object) [minItems=1, maxItems=1] **REQ** — Single contact role relation definition
    - `Contact_Role` (object) **REQ** — Contact role details to associate
      - `name` (string) [maxLen=255] — Contact role display name
      - `id` (string/int64) — Unique identifier of the contact role

**Responses:**

- **200**: Represents the response of a successfully associated the contact role with the deal. [application/json]
    > Provides the response of updating the contact role relation.
    - `data` (array of object) [minItems=1, maxItems=1] **REQ** — Status entries for the processed contact relation
      - `code` (string) **REQ** [const=SUCCESS] — Operation status code
      - `details` (object) **REQ** — Details about the processed relation
        - `id` (string/int64) **REQ** — Identifier of the contact-deal relation
      - `message` (string) **REQ** [const=relation updated] — Result message
      - `status` (string) **REQ** [const=success] — Operation status

- **400**: Bad Request - Invalid data, invalid module, field limit exceeded, or required parameter missing [application/json]
...
- **403**: Represents the error response when the user does not have permission to access deals or contacts. [application/json]
    > Error response for when the user lacks permission to access deals or contacts.
    oneOf:
