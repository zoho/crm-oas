...
**Operation:** `deassociateUsersFromTerritories` — Territory Users
...
- `ids` (query, string, required) [maxLen=500]: Ids to be removed from territory
...
- **400**: Returns an error when the request contains invalid user IDs or missing required parameters. **Resolution:** Verify the user IDs and ensure all required parameters are provided. [application/json]
    > Represents the error response body returned when the request contains invalid parameters.
    oneOf:
      - `UserInvalidErrorSchemaOnRequestBody` — Represents the invalid-request error schema returned when one or more user IDs in the request fail validation.
