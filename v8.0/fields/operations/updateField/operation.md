...
**Operation:** `updateField` — Update custom fields
> To update one or more custom fields in a module in your Zoho CRM account.

**Parameters:**
- `module` (query, string, required) [maxLen=30] {style=form, explode=True}: Specify the API name of the module for which you want to update custom fields. For example, the module name 'Leads'.
...
- **200**: Returns the update status for each field submitted in the request, with the unique ID of each successfully updated field. — Schema: `CreateFieldsResponse` [application/json]
...
- **207**: Returns a partial-success response where some fields were updated successfully and others failed validation, with per-field status information. [application/json]
    > Represents the multi-status response for a bulk field update request, containing a mix of success and error results.
    - `fields` (array of object) [minItems=2, maxItems=25] **REQ** — Represents the list of per-field results for the bulk update operation, where each item indicates either a successful update or a validation error.
      oneOf:
        - `FieldCreationResult` — Represents the result of a single field creation.
        - `ErrorResponse` — Represents the error response structure.

- **400**: The request to update one or more fields is invalid. Resolution: The request payload and module API name must be valid, every field object must include a valid field **id** and satisfy the schema for its declared **data_type**, and the field must not be system-defined, undergoing type conversion, or subject to association limits. [application/json]
    oneOf:
      - `ErrorResponse` — Represents the error response structure.
        - `fields` (array of object `ErrorResponse`) [maxItems=25] **REQ** — Represents the array of per-field error objects when validation fails for one or more fields in the update request.
