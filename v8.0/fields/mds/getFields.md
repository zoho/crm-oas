# GET /settings/fields
**Operation:** `getFields` — List fields metadata
...
- **200**: Returns the field metadata for all fields in the specified module, including standard, custom, subform, and linking module fields. [application/json]
    > Represents the response body for the field metadata request, containing an array of field configuration objects.
    - `fields` (array of object `FieldSchema`) [maxItems=100] **REQ** — Represents the list of field metadata objects retrieved for the specified module.
      schema: `FieldSchema`
      - `mask_details` (object)
        oneOf:
...
- **204**: No content. The request was processed successfully, and the response body is empty.

- **400**: The module API name in the request is invalid. Resolution: The **module** query parameter must reference a valid module API name. — Schema: `InvalidModuleName` [application/json]
    > Represents the error response returned when an invalid module name is provided in the request.
    schema: `InvalidModuleName`
    - `code` (string) **REQ** [maxLen=50, pattern=^[A-Z_]+$] — Represents the error code indicating the type of failure.
    - `details` (object) **REQ** — Represents additional details about the invalid module name error.
    - `message` (string) **REQ** [maxLen=255] — Represents the error message describing the issue.
    - `status` (string) **REQ** [maxLen=50] — Represents the status of the response.
