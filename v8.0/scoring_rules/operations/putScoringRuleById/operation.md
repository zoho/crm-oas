...
**Operation:** `putScoringRuleById` — Update a scoring rules by ID
...
**Request Body** — application/json `UpdateScoringRulesByIDRequestBody`
> Accepts a JSON request body containing the updated Scoring Rule definition.
  > Represents the request body schema for bulk update Scoring Rules operations.
  - `scoring_rules` (array of object `ScoringRulePUT`) [maxItems=10] **REQ** — Represents an array of Scoring Rule update objects. Required for PUT operations.
...
- **200**: Returns the operation status after updating the specified Scoring Rule. — Schema: `ScoringRulePutSuccessResponse` [application/json]
    > Rule successfully updated
    schema: `ScoringRulePutSuccessResponse`
    - `scoring_rules` (array of object `ScoringRuleResponseDetails`) [maxItems=1] — Update result for the scoring rule, containing the status and ID of the updated rule.
...
**Resolution:** Verify that all field values are valid and required fields are present. [application/json]
    ...
    - `scoring_rules` (array of object `GeneralErrorResponse`) [maxItems=25] **REQ** — Represents an array of per-item error objects in the update Scoring Rule by ID error response.
