...
**Operation:** `putActivate` — Activate a scoring rule
...
> Accepts an empty JSON request body to activate the specified Scoring Rule.
  > Empty request body

**Responses:**

- **200**: Returns the operation status after the Scoring Rule is activated. — Schema: `ScoringRuleActivateSuccessResponse` [application/json]
    > Scoring rule successfully activated
    schema: `ScoringRuleActivateSuccessResponse`
    - `scoring_rules` (array of object `ScoringRuleResponseDetails`) [maxItems=1] — Activation result for the scoring rule, containing the status and details of the activated rule.
