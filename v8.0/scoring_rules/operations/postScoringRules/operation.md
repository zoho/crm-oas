...
**Operation:** `postScoringRules` — Scoring Rules
...
**Request Body** — application/json `CreateScoringRuleRequestBody`
> Accepts a JSON request body containing an array of Scoring Rule definitions to create.
  > Represents the request body for the create Scoring Rules operation, containing an array of Scoring Rule definitions.
  - `scoring_rules` (array of object `ScoringRulePOST`) [minItems=1, maxItems=20] **REQ** — Represents an array of Scoring Rule definitions to create. Required for POST operations.
    schema: `ScoringRulePOST`
    - `custom_fields` (array of object `CustomFieldConfiguration`) [maxItems=6, nullable] — Custom fields can be configured for scoring rule. These fields will hold the referred scores.
      ...
      - `reference_field` (object `ReferencedScoreField`) — It will hold the referenced score field info. (Required for POST)
        ...
        additionalProperties: any
    - `module` (object `ScoringRuleModuleInput`) **REQ** — Provides details of a module, including API name and module ID.
      schema: `ScoringRuleModuleInput`
      - `api_name` (string) [maxLen=255] — Api name of the module
      - `id` (string/int64) [maxLen=25] — Represents the module id
    ...
    - `layout` (object `ScoringRuleLayoutInput`) — The layout this scoring rule applies to. Omit or pass null to apply the rule to all layouts. When provided as an object, id and api_name are required.
      schema: `ScoringRuleLayoutInput`
      - `display_label` (string) [maxLen=255] — layout display name
      - `api_name` (string) [maxLen=255, minLen=1, pattern=^[A-Za-z][A-Za-z0-9_]*$] — Layout api name
      - `id` (string/int64) [maxLen=25, minLen=1, pattern=^[0-9]{1,19}$] — Layout id
    - `field_rules` (array of object `FieldScoreConfigurationRequest`) [maxItems=50] — Specifies an array of field-based score configurations that define criteria and associated scores.
      ...
      - `criteria` (object `FilterCriterionRequest`) — Represents the top-level filter criterion for data selection, supporting both simple field-based filters and complex grouped filters with logical operators.
        oneOf:
          - `SimpleFilterCriterionRequest` — Represents a simple field-based filter criterion that dispatches to the appropriate comparator-specific schema. The equal and not_equal comparators support scalar or array values; text comparators support scalar or array strings; ordering comparators require a scalar; between and not_between comparators require a two-element array; in and not_in comparators require an array of primitives; and include_all, include_any, exclude_all, and exclude_any comparators require an array of lookup objects and apply only when the field's data_type is multiuserlookup or multiselectlookup.
          - `GroupedFilterCriterionRequest` — Represents a grouped filter criterion containing multiple filter conditions evaluated with a logical operator. Supports recursive nesting for complex filtering logic.
    - `name` (string) **REQ** [maxLen=25] — Scoring rule name. Must be unique within the organization. It cannot contain special characters: ` ~ # % ^ & * ( ) + = " ; < > [ ] { } | \
    - `signal_rules` (array of object `TouchPointScoreConfigurationPOST`) [maxItems=250, nullable] — Represents the touch points based score configurations and is applicable only for Leads and Contacts modules
      schema: `TouchPointScoreConfigurationPOST`
      - `score` (integer/int32) — Points that should be added for the signal (Required)
      - `id` (string) [maxLen=25] — Signal rule primary key
      - `signal` (object `SignalInformation`) — Touch point signal info (Required)
...
- **201**: Scoring rule is created successfully, but one or more custom fields are not created. — Schema: `CreateScoringRuleSuccessResponse` [application/json]
    > Successful creation response for scoring rules, containing details of the created rules.
    schema: `CreateScoringRuleSuccessResponse`
    - `scoring_rules` (array of object `CreatedScoringRuleResponseItem`) [maxItems=20] — Represents the response body for a successful create Scoring Rules operation.
      schema: `CreatedScoringRuleResponseItem`
      - `code` (string) [maxLen=255] — Represents the response code for the scoring rule creation operation
      - `details` (object) — Details of an operation, including the identifier of affected fields.
        - `id` (string) [maxLen=25] — Represents the created scoring rule id
      - `message` (string) [maxLen=255] — Represents the response message for the scoring rule creation operation
      - `status` (string) [maxLen=255] — Represents th response status for the scoring rule creation operation

- **207**: Returns per-item operation statuses when some Scoring Rule items succeed and others fail in the same request. — Schema: `PartialSuccessResponse` [application/json]
    > Represents the multi-status response body for the create Scoring Rules operation when some items succeed and others fail.
    schema: `PartialSuccessResponse`
    - `scoring_rules` (array of object `ScoringRuleResponseDetails`) [maxItems=20] — Represents an array of per-item operation statuses for the partial success response.
...
**Resolution:** Verify that all field values are valid and required fields are present. [application/json]
    ...
    - `scoring_rules` (array of object `GeneralErrorResponse`) [maxItems=250] **REQ** — Represents an array of per-item error objects in the create Scoring Rules error response.
