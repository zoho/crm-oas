...
**Operation:** `deleteScoringRuleById` — Delete a scoring rules by ID
...
- **200**: Returns the operation status after deleting the specified Scoring Rule. — Schema: `ScoringRuleDeleteSuccessResponse` [application/json]
    > successful deletion of a rule
    schema: `ScoringRuleDeleteSuccessResponse`
    - `scoring_rules` (array of object `ScoringRuleResponseDetails`) [maxItems=1] — Deletion result for the scoring rule, containing the status and ID of the deleted rule.
