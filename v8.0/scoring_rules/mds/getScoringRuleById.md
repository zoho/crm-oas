...
**Operation:** `getScoringRuleById` — Get scoring rule by ID
...
- `include_inner_details` (query, string, optional) [maxLen=255]: Comma-separated list of inner detail fields to include in the response. Supported values: custom_fields.field_label, custom_fields.reference_field.
...
- **200**: Returns the details of the Scoring Rule matching the specified ID. — Schema: `SuccessResponseGETById` [application/json]
    > Scoring rule details
    schema: `SuccessResponseGETById`
    - `scoring_rules` (array of object `ScoringRuleGET`) [maxItems=1] — Represents an array containing the Scoring Rule details returned for the get by ID operation.
...
- **204**: No rule is available for the given rule id
