# Examples: getRecords

**GET /{module}**

## Parameter examples

### `module` (path) — Example

```json
"Leads"
```

## Response examples

### Status `200` — `application/json` — Leads

Successful retrieval of records from the Leads module

```json
{
  "data": [
    {
      "id": "4150868000000002175",
      "Owner": {
        "name": "Patricia Boyle",
        "id": "4150868000000225013",
        "email": "patricia.boyle@example.com"
      },
      "Company": "Zylker Corp",
      "First_Name": "John",
      "Last_Name": "Doe",
      "Email": "john.doe@zylker.com",
      "Phone": "+1-555-123-4567",
      "Lead_Status": "Not Contacted",
      "Created_Time": "2023-10-15T10:30:00+00:00",
      "Modified_Time": "2023-10-15T14:20:00+00:00"
    }
  ],
  "info": {
    "call": false,
    "per_page": 5,
    "next_page_token": "c8582xx9e7c7",
    "count": 5,
    "sort_by": "id",
    "page": 1,
    "page_token_expiry": "2022-11-11T15:08:14+05:30",
    "sort_order": "desc",
    "email": false,
    "more_records": true,
    "previous_page_token": null
  }
}
```

### Status `400` — `application/json` — InvalidData

Invalid module API name in the request URL

```json
{
  "code": "INVALID_MODULE",
  "details": {
    "resource_path_index": 0
  },
  "message": "the module name given seems to be invalid",
  "status": "error"
}
```

### Status `400` — `application/json` — InvalidCvid

Invalid Custom View ID in the cvid parameter

```json
{
  "code": "INVALID_DATA",
  "details": {
    "param_name": "cvid"
  },
  "message": "invalid data",
  "status": "error"
}
```

### Status `400` — `application/json` — TokenBoundDataMismatch

Page token mismatch due to changed request parameters

```json
{
  "code": "TOKEN_BOUND_DATA_MISMATCH",
  "message": "The page_token given seems to be invalid or input param is added or altered or deleted",
  "status": "error",
  "details": {
    "param_name": "page_token"
  }
}
```

### Status `400` — `application/json` — RequiredParamMissing

Required fields parameter missing from the request

```json
{
  "code": "REQUIRED_PARAM_MISSING",
  "status": "error",
  "details": {
    "param_name": "fields"
  },
  "message": "required param is missing"
}
```

### Status `400` — `application/json` — LimitExceeded

Field count exceeds the maximum allowed limit

```json
{
  "code": "LIMIT_EXCEEDED",
  "details": {
    "limit": 50,
    "param_name": "fields"
  },
  "message": "limit exceeded",
  "status": "error"
}
```

### Status `400` — `application/json` — ExpiredValue

Expired page_token parameter value

```json
{
  "status": "error",
  "code": "EXPIRED_VALUE",
  "message": "Page token expired",
  "details": {
    "api_name": "page_token",
    "json_path": "$.query.page_token"
  }
}
```

### Status `400` — `application/json` — DiscretePaginationLimitExceeded

Pagination limit exceeded without the page_token parameter

```json
{
  "code": "DISCRETE_PAGINATION_LIMIT_EXCEEDED",
  "message": "You can only get the first 2000 records without using page_token param",
  "status": "error",
  "details": {
    "limit": 2000
  }
}
```

### Status `400` — `application/json` — PaginationLimitExceeded

Pagination limit of 100,000 records exceeded

```json
{
  "code": "PAGINATION_LIMIT_EXCEEDED",
  "message": "You can only get upto 100000 records using page_token param",
  "status": "error",
  "details": {
    "param_name": "page_token"
  }
}
```

### Status `401` — `application/json` — ScopeMismatch

OAuth access token missing the required read scope

```json
{
  "code": "OAUTH_SCOPE_MISMATCH",
  "message": "Unauthorized",
  "status": "error",
  "details": {}
}
```

### Status `403` — `application/json` — NoPermission

User profile lacks module access permission

```json
{
  "code": "NO_PERMISSION",
  "message": "Permission denied",
  "status": "error",
  "details": {
    "permissions": [
      "Crm_Implied_View_Leads"
    ]
  }
}
```

### Status `404` — `application/json` — InvalidUrlPattern

Invalid or unrecognized API endpoint URL pattern

```json
{
  "code": "INVALID_URL_PATTERN",
  "message": "Please check if the URL trying to access is a correct one",
  "status": "error",
  "details": {}
}
```

### Status `500` — `application/json` — InternalError

Unexpected internal server error during request processing

```json
{
  "code": "INTERNAL_ERROR",
  "message": "Internal Server Error",
  "status": "error",
  "details": {}
}
```
