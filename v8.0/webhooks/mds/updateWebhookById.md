...
**Operation:** `updateWebhookById` — Update webhook by ID
> To update the configuration of a specific webhook in your Zoho CRM organization using its ID.

The module field cannot be changed after webhook creation. Read-only webhooks created by Marketplace extensions cannot be updated.

**Parameters:**
- `webhookId` (path, string, required) [maxLen=20]: Specify the unique ID of the webhook to update. Refer to the [Get Webhooks](webhooks.yaml#$.paths./settings/automation/webhooks.get) resource for valid values.
...
- **200**: Returns the result of the webhook update request for the specified webhook ID, including the webhook ID in the details object. — Schema: `UpdateWebhookSuccessResponse` [application/json]
...
- **400**: One or more request body fields are invalid. Returned for validation errors including invalid webhook ID, module change attempt, read-only webhook, unsupported HTTP method, malformed URL, and data type or length violations.

**Resolution:** All required fields must be present and valid. The module field cannot be changed after webhook creation. Read-only webhooks created by extensions cannot be updated. [application/json]
    > Contains the error response for an invalid webhook update-by-ID request.

The schema is a union of error variants covering field-level validation, module change restrictions, read-only webhook violations, data type errors, length violations, and supported value violations.
    oneOf:
        - `webhooks` (array of object) [maxItems=25] **REQ** — Represents the list of error objects for the invalid webhook in the update-by-ID request.
          oneOf:
...
- **403**: Permission denied to update webhooks in this Zoho CRM organization.
