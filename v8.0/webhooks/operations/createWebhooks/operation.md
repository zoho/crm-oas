...
**Operation:** `createWebhooks` — Webhooks
> To create a webhook action that fires an HTTP request to an external URL in your Zoho CRM organization when an automation trigger fires. 

The `http_method` value controls which optional request fields are accepted. 

- POST and PUT accept `headers` and `body`
- GET and DELETE methods accept `url_parameters`
- DELETE also accepts `headers` 
...
`InvalidDataSupportedValuesError`:
  > Represents the error returned when a field value is not among the accepted values for that field. The details object includes a supported_values array listing the accepted values.
  - `code` (string) **REQ** [enum=['INVALID_DATA']] — Represents the error code for this failure.
Possible values:
...
Possible values:
**error** - Indicates the request did not complete successfully.
`MandatoryParamMissingError`:
  > Represents the error returned when a mandatory request body field is absent. The details object includes the missing field's api_name and json_path.
  - `code` (string) **REQ** [enum=['MANDATORY_NOT_FOUND']] — Represents the error code for this failure.
Possible values:
**MANDATORY_NOT_FOUND** - A required query or path parameter is missing from the request.
  - `details` (object) **REQ** — Represents the error details identifying the mandatory parameter that is missing.
    - `api_name` (string) [maxLen=255] — Represents the API name of the mandatory parameter that is missing.
    - `json_path` (string) [maxLen=1000] — Represents the JSON path to the mandatory parameter that is missing.
  - `message` (string) **REQ** [enum=['mandatory param missing', 'required field not found']] — Represents the error message describing the failure.

**Possible values**:
**mandatory param missing** - A required parameter was not provided in the request.
**required field not found** - A required field is absent from the request.
  - `status` (string) **REQ** [enum=['error']] — Represents the error status.
Possible values:
**error** - Indicates the request did not complete successfully.
...
**Request Body** — application/json `CreateWebhooksRequestBody`
> The request body must contain a webhooks array with one object. The webhook object requires name, module, url, http_method, authentication, and feature_type.
  > Represents the request body wrapper for creating a webhook. The webhooks array must contain exactly one webhook object.
  - `webhooks` (array of object `WebhookCreateInput`) [minItems=1, maxItems=1] **REQ** — Represents the list containing exactly one webhook object to create.
    schema: `WebhookCreateInput`
    - `name` (string) **REQ** [maxLen=250] — Specify the display name of the webhook. The name must not contain the tilde (~) character.
    - `module` (object `AutomationModuleDetailsResponse`) **REQ** — The CRM module this webhook is associated with. Provide at least one of api_name or id. Cannot be changed after creation.
    ...
    - `http_method` (string) **REQ** [enum=['POST', 'GET', 'PUT', 'DELETE']] — Specify the HTTP method used when the webhook fires. POST and PUT accept headers and body. GET and DELETE accept url_parameters. DELETE also accepts headers.
Possible values:
**POST** - Sends a POST request.
**GET** - Sends a GET request.
**PUT** - Sends a PUT request.
**DELETE** - Sends a DELETE request.
    - `authentication` (object `WebhookAuthenticationDetails`) **REQ** — Authentication configuration. Required for all webhooks, including those without authentication credentials. Use type=general for webhooks that do not require authentication (no connection_name needed). Use type=connection when the webhook endpoint requires authenticated access via a pre-configured connection (connection_name is required).
...
**connection** - Uses a pre-configured Zoho connection.
    ...
    - `related_module` (object `RelatedModuleReference`) — Related CRM module. Required when feature_type is kiosk. Null otherwise.
...
**commandcenter_service** - CommandCenter service.
    - `headers` (object `WebhookHeaders`) — HTTP headers for the outgoing webhook request. Only allowed when http_method is POST, PUT, or DELETE. Restricted header name: HOST is not allowed. Parameter names must be unique across the collection (DUPLICATE_DATA).
    ...
    - `url_parameters` (object `WebhookUrlParameters`) — URL query parameters for the outgoing request. Only allowed when http_method is GET or DELETE.
...
**none** - No body content.
    ...
    - `date_time_format` (object `DateTimeFormatConfig`) — Optional. Only relevant when a date or datetime merge-field is used in any parameter (headers, url_parameters, or body). Use datetime_format + time_zone when a datetime field is referenced (e.g., ${!Leads.Created_Time}). Use date_format when a date field is referenced (e.g., ${!Leads.Date_1}). Ignored if no date/datetime merge-fields are present.
...
- **201**: Returns an array containing the operation result for the created webhook, including the new webhook ID. — Schema: `CreateWebhooksSuccessResponse` [application/json]
    > Represents the success response for a webhook creation request, including the operation result and the new webhook ID.
    schema: `CreateWebhooksSuccessResponse`
    - `webhooks` (array of object `WebhookStatusResponse`) [maxItems=1] — Represents the list containing the operation result for the created webhook, including the new webhook ID.
...
- **400**: One or more request body fields are invalid.

**Resolution:** The error details in the response body must be reviewed to identify the invalid field, and the value must be corrected before retrying. [application/json]
    > Represents the error response body for a create webhook validation failure. Contains one of several wrapped error structures, each with a webhooks array or a direct error object.
    oneOf:
        - `webhooks` (array of object) [maxItems=25] **REQ** — Represents the array of validation error objects returned for the create request.
          oneOf:
            - `MandatoryParamMissingError` — Represents the error returned when a mandatory request body field is absent. The details object includes the missing field's api_name and json_path.
...
**error** - Indicates the request did not complete successfully.
            ...
            - `ExpectedFieldMissingError` — Represents the error returned when a required object is supplied but does not include any of the expected identifying fields.
              - `code` (string) **REQ** [enum=['EXPECTED_FIELD_MISSING']] — Represents the error code for this failure.
Possible values:
**EXPECTED_FIELD_MISSING** - One or more expected fields are absent from the request.
              - `details` (object) **REQ** — Represents the error details listing the fields of which at least one must be provided.
                - `expected_fields` (array of object) [maxItems=5] — Represents the list of expected fields of which at least one must be provided.
                  - `api_name` (string) [maxLen=255] — Represents the API name of the expected field.
                  - `json_path` (string) [maxLen=1000] — Represents the JSON path to the expected field.
              - `message` (string) **REQ** [enum=['Specify atleast one field', 'Provide the expected field']] — Represents the error message describing the failure.
Possible values:
**Specify atleast one field** - At least one of the expected fields must be provided in the request.
**Provide the expected field** - A specific required field is absent from the request.
              - `status` (string) **REQ** [enum=['error']] — Represents the error status.
Possible values:
**error** - Indicates the request did not complete successfully.
            - `DependentFieldMissingError` — Represents the error returned when a field that is required by another field's value is absent from the request.
              - `code` (string) **REQ** [enum=['DEPENDENT_FIELD_MISSING']] — Represents the error code for this failure.
Possible values:
**DEPENDENT_FIELD_MISSING** - A required dependent field is missing from the request.
              - `details` (object) **REQ** — Represents the error details containing information about the missing dependent field and the dependee field that triggered the requirement.
                - `dependee` (object) — Represents the dependee field whose value triggers the dependency requirement.
                  - `api_name` (string) [maxLen=255] — Represents the API name of the dependee field.
                  - `json_path` (string) [maxLen=1000] — Represents the JSON path to the dependee field.
                - `api_name` (string) [maxLen=255] — Represents the API name of the missing or conflicting field.
                - `json_path` (string) [maxLen=1000] — Represents the JSON path to the missing or conflicting field.
              - `message` (string) **REQ** [enum=[4 values]] — Represents the error message describing the failure.
Possible values:
**raw data content key is mandatory for raw body type** - The raw_data_content field is required when the body type is raw.
**form data content key is mandatory for form body type** - The form_data_content field is required when the body type is form.
**Webhook action related module missing for kiosk feature** - A related module is required for kiosk-type webhooks.
**Dependent Field missing** - A required dependent field was not included in the request.
              - `status` (string) **REQ** [enum=['error']] — Represents the error status.
Possible values:
**error** - Indicates the request did not complete successfully.
            - `EmptyUrlError` — Represents the error returned when the url field is present in the request but contains an empty string.
              - `code` (string) **REQ** [enum=['INVALID_DATA']] — Represents the error code for this failure.
Possible values:
**INVALID_DATA** - The webhook URL field is empty or not provided.
              - `details` (object) **REQ** — Represents the error details identifying the field with the empty value.
                - `api_name` (string) [maxLen=255] — Represents the API name of the field that contains the empty value.
                - `json_path` (string) [maxLen=1000] — Represents the JSON path to the field that contains the empty value.
              - `message` (string) **REQ** [enum=['Empty value is not allowed']] — Represents the error message describing the failure.
Possible values:
**Empty value is not allowed** - The webhook URL field cannot be empty.
              - `status` (string) **REQ** [enum=['error']] — Represents the error status.
Possible values:
**error** - Indicates the request did not complete successfully.
            - `SsrfUrlError` — Represents the error returned when the URL provided fails SSRF validation. Internal network addresses and private host ranges are not permitted.
              - `code` (string) **REQ** [enum=['INVALID_DATA']] — Represents the error code for this failure.
Possible values:
**INVALID_DATA** - The webhook URL is blocked due to SSRF protection. It resolves to an internal or private host address.
              - `details` (object) **REQ** — Represents the error details identifying the field with the insecure URL.
                - `api_name` (string) [maxLen=255] — Represents the API name of the URL field that contains the insecure address.
                - `json_path` (string) [maxLen=1000] — Represents the JSON path to the URL field that contains the insecure address.
              - `message` (string) **REQ** [enum=["Url you've configured is not secure"]] — Represents the error message describing the failure.
Possible values:
**URL you've configured is not secure** - The webhook URL resolves to an internal or private host address that is blocked by SSRF protection.
              - `status` (string) **REQ** [enum=['error']] — Represents the error status.
Possible values:
**error** - Indicates the request did not complete successfully.
            - `InvalidFeatureTypeError` — Represents the error returned when the feature_type value in the request body is not a recognized automation feature type.
              - `code` (string) **REQ** [enum=['INVALID_DATA']] — Represents the error code for this failure.
Possible values:
**INVALID_DATA** - The feature type name provided in the request is not valid.
              - `details` (object) **REQ** — Represents the error details identifying the field with the invalid feature type name.
                - `api_name` (string) [maxLen=255] — Represents the API name of the field with the invalid feature type value.
                - `json_path` (string) [maxLen=1000] — Represents the JSON path to the field with the invalid feature type value.
              - `message` (string) **REQ** [enum=['the given feature name is invalid']] — Represents the error message describing the failure.
Possible values:
**the given feature name is invalid** - The feature type value does not match any supported automation feature type.
              - `status` (string) **REQ** [enum=['error']] — Represents the error status.
Possible values:
**error** - Indicates the request did not complete successfully.
            - `InvalidBodyTypeError` — Represents the error returned when body.type contains a value other than form_data, raw, or none.
              - `code` (string) **REQ** [enum=['INVALID_DATA']] — Represents the error code for this failure.
Possible values:
**INVALID_DATA** - The body type value provided in the request is not supported.
              - `details` (object) **REQ** — Represents the error details identifying the field with the invalid field type value.
                - `api_name` (string) [maxLen=255] — Represents the API name of the field with the invalid body type value.
                - `json_path` (string) [maxLen=1000] — Represents the JSON path to the field with the invalid body type value.
              - `message` (string) **REQ** [enum=['invalid body type passed']] — Represents the error message describing the failure.
Possible values:
**invalid body type passed** - The body type value is not among the supported options.
              - `status` (string) **REQ** [enum=['error']] — Represents the error status.
Possible values:
**error** - Indicates the request did not complete successfully.
            - `InvalidBodyFormatError` — Represents the error returned when body.format contains an unsupported value for the raw body type.
              - `code` (string) **REQ** [enum=['INVALID_DATA']] — Represents the error code for this failure.
Possible values:
**INVALID_DATA** - The body format value provided in the request is not valid.
              - `details` (object) **REQ** — Represents the error details identifying the field with the invalid body format value.
                - `api_name` (string) [maxLen=255] — Represents the API name of the field with the invalid format value.
                - `json_path` (string) [maxLen=1000] — Represents the JSON path to the field with the invalid format value.
                - `regex` (string) [maxLen=1000] — Represents the regex pattern used to validate the body format field.
              - `message` (string) **REQ** [enum=['invalid format passed in body']] — Represents the error message describing the failure.
Possible values:
**invalid format passed in body** - The format value provided in the body field does not match the expected pattern.
              - `status` (string) **REQ** [enum=['error']] — Represents the error status.
Possible values:
**error** - Indicates the request did not complete successfully.
            - `InvalidFormDataContentError` — Represents the error returned when body.form_data_content contains invalid or malformed data.
              - `code` (string) **REQ** [enum=['INVALID_DATA']] — Represents the error code for this failure.
Possible values:
**INVALID_DATA** - The form data content in the request is not valid.
              - `details` (object) **REQ** — Represents the error details identifying the field with the invalid form data content.
                - `api_name` (string) [maxLen=255] — Represents the API name of the field with the invalid form data content.
                - `dependee` (object) — Represents the dependee field object that triggered the form data content requirement.
                  - `api_name` (string) [maxLen=255] — Represents the API name of the dependee field.
                  - `json_path` (string) [maxLen=1000] — Represents the JSON path to the dependee field.
                - `json_path` (string) [maxLen=1000] — Represents the JSON path to the field with the invalid form data content.
              - `message` (string) **REQ** [enum=[3 values]] — Represents the error message describing the failure.
              - `status` (string) **REQ** [enum=['error']] — Represents the error status.
Possible values:
**error** - Indicates the request did not complete successfully.
            - `NotAllowedErrorResponse` — Represents the error returned when the specified module is not supported for webhook operations.
              - `code` (string) **REQ** [enum=['NOT_ALLOWED']] — Represents the error code for this failure.
Possible values:
**NOT_ALLOWED** - The operation is not allowed for the specified module.
              - `details` (object) **REQ** — Represents the error details identifying the unsupported module or resource.
                - `api_name` (string) [maxLen=255] — Represents the API name of the unsupported module.
                - `json_path` (string) [maxLen=1000] — Represents the JSON path to the unsupported module field.
              - `message` (string) **REQ** [enum=['The given module is not supported']] — Represents the error message describing the failure.
Possible values:
**The given module is not supported** - The module specified in the request does not support webhook automation.
              - `status` (string) **REQ** [enum=['error']] — Represents the error status.
Possible values:
**error** - Indicates the request did not complete successfully.
            - `LimitExceededError` — Represents the error returned when the number of items in a parameter array exceeds the allowed limit.
              - `code` (string) **REQ** [enum=['LIMIT_EXCEEDED']] — Represents the error code for this failure.
Possible values:
**LIMIT_EXCEEDED** - The number of webhooks exceeds the maximum allowed for the organization.
              - `details` (object) **REQ** — Represents the error details containing the field name and the enforced limit.
                - `api_name` (string) [maxLen=255] — Represents the API name of the field for which the limit is exceeded.
                - `limit` (integer/int32) — Represents the maximum number of items allowed for the field.
                - `json_path` (string) [maxLen=1000] — Represents the JSON path to the field for which the limit is exceeded.
              - `message` (string) **REQ** [enum=["can't pass more than the limit"]] — Represents the error message describing the failure.
Possible values:
**can't pass more than the limit** - The submitted value exceeds the allowed limit for this field.
              - `status` (string) **REQ** [enum=['error']] — Represents the error status.
Possible values:
**error** - Indicates the request did not complete successfully.
            - `InvalidDataSupportedValuesError` — Represents the error returned when a field value is not among the accepted values for that field. The details object includes a supported_values array listing the accepted values.
...
- **403**: The requesting user does not have the Manage Workflow permission, or the current CRM edition does not support webhooks.

**Resolution:** The CRM administrator must grant the Manage Workflow permission to the user's profile, or the account must be on an edition that supports webhooks. [application/json]
    > Represents the error response for permission or feature-support issues encountered during webhook creation.
    oneOf:
      - `NoPermissionErrorResponse` — Represents the error returned when the requesting user does not have the required CRM profile permission to perform the operation.
...
Possible values:
**error** - Indicates the request did not complete successfully.
      - `FeatureNotSupportedError` — Represents the error returned when the current CRM edition does not support the webhook feature.
        - `code` (string) **REQ** [enum=['FEATURE_NOT_SUPPORTED']] — Represents the error code for this failure.
Possible values:
**FEATURE_NOT_SUPPORTED** - The requested feature is not available in the current edition.
        - `details` (object) **REQ** — Represents the error details containing additional context about the unsupported feature.
        - `message` (string) **REQ** [enum=['feature not available in this edition']] — Represents the error message describing the failure.
Possible values:
**feature not available in this edition** - The requested webhook feature is not supported by the current CRM edition.
        - `status` (string) **REQ** [enum=['error']] — Represents the error status.
Possible values:
**error** - Indicates the request did not complete successfully.
