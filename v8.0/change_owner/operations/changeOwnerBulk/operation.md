...
**Operation:** `changeOwnerBulk` — Change Owner
...
- `ids` (query, string, optional) [maxLen=9975]: Deprecated — use the `ids` array in the request body instead.
...
**Request Body** (required) — application/json
  ...
  - `ids` (array of string) [maxItems=500] **REQ** — Specify the IDs of the records for which you want to change the owner.
    items: [maxLen=19]
  - `owner` (object) **REQ** — Specify the details of the new owner for the records.
    - `id` (string) **REQ** [maxLen=19] — Specify the user ID of the new owner.
  - `notify` (boolean) [nullable] — Specify whether to notify the new owner via email when the records are assigned.\nPossible values:\n**true** - The new owner is notified via email.\n**false** - The new owner is not notified via email.
...
- **200**: Returns the result of the owner change operation for the requested records. [application/json]
    > Successful owner change response
    - `data` (array of object) [maxItems=500] — Data containing the result of the owner change operation
      - `code` (string) **REQ** [enum=['SUCCESS']] — Response code indicating success
      - `details` (object) **REQ** — Details of the updated record
        - `id` (string) **REQ** [maxLen=19] — The ID of the updated record
      - `message` (string) **REQ** [maxLen=500] — Success message
      - `status` (string) **REQ** [enum=['success']] — Status of the operation

- **202**: Partial success (direct API calls) — one or more records failed while others succeeded. Same body shape as 207. [application/json]
    > Partial bulk change-owner response returned to direct API callers.
    - `data` (array of object) [maxItems=500] — Per-record outcome array containing a mix of success and error objects.
      - `code` (string) [maxLen=100] — Outcome code for this record. `SUCCESS` on success; an error code such as `CANNOT_PROCESS`, `NOT_MERGED`, or `CANNOT_PERFORM_ACTION` on failure.
      - `status` (string) [enum=['success', 'error']] — Status of the operation for this record.
      additionalProperties: any

- **207**: Partial success (client SDK calls) — one or more records failed while others succeeded. Each element in `data` carries an individual `code` and `status` indicating the per-record outcome. [application/json]
    > Partial bulk change-owner response returned to client SDK callers.
    - `data` (array of object) [maxItems=500] — Per-record outcome array containing a mix of success and error objects.
      - `code` (string) [maxLen=100] — Outcome code for this record. `SUCCESS` on success; an error code such as `CANNOT_PROCESS`, `NOT_MERGED`, or `CANNOT_PERFORM_ACTION` on failure.
      - `status` (string) [enum=['success', 'error']] — Status of the operation for this record.
      additionalProperties: any

- **400**: Invalid request. The module is unsupported, the data is invalid, or a required field is missing.\n**Resolution:** The module API name in the request URL must be valid, the record IDs must exist, and the owner ID must be specified correctly. [application/json]
    oneOf:
        ...
        - `details` (object) **REQ** — Represents additional context about the error.
          ...
          - `supported_version` (integer/int32) — Minimum supported API version. Returned with the API_NOT_SUPPORTED error code.
        ...
        - `data` (object) **REQ** — Error details for invalid data type.
          - `code` (string) **REQ** [enum=['INVALID_DATA']] — Error code indicating the type of error
          - `details` (object) **REQ** — Additional details about the data type mismatch
            - `expected_data_type` (string) **REQ** [maxLen=50] — Denotes the expected data type of the field.
            - `api_name` (string) **REQ** [maxLen=200] — Represents the API name of the field that caused the error.
            - `json_path` (string) **REQ** [maxLen=500] — JSON path to the field that caused the error
          - `message` (string) **REQ** [maxLen=500] — Error message describing the issue
          - `status` (string) **REQ** [enum=['error']] — Represents the status of the response
        - `data` (array of object) [maxItems=500] **REQ** — List of records that are locked
          - `code` (string) **REQ** [enum=['RECORD_LOCKED']] — Error code indicating the type of error
          - `details` (object) **REQ** — Additional details about the locked record
            - `action` (string) **REQ** [maxLen=100] — The action that caused the locking conflict
            - `id` (string) **REQ** [maxLen=19] — Represents the ID of the locked record
          - `message` (string) **REQ** [maxLen=500] — Inidcates the error message describing the issue
          - `status` (string) **REQ** [enum=['error']] — Denotes the status of the response
    ...
    oneOf:
        ...
        - `details` (object) **REQ** — Represents the additional details about the missing permissions.
          - `permissions` (array of string) [maxItems=1] **REQ** — Represents the list of required permissions that the user's profile is missing.
            items: [maxLen=200]
        - `code` (string) **REQ** [enum=['CANNOT_ASSIGN']] — Error code indicating a permission gap on the new owner. Possible values: **CANNOT_ASSIGN** - The new owner does not have view access to the required module.
        - `details` (object) **REQ** — Additional context listing the missing permissions.
          - `permissions` (array of string) [maxItems=10] **REQ** — List of permissions that the new owner lacks.
            items: [maxLen=200]
        - `message` (string) **REQ** [maxLen=500] — Error message describing the missing module permission.
        - `status` (string) **REQ** [enum=['error']] — Response status. Possible values: **error** - The operation did not complete.
        - `code` (string) **REQ** [enum=['CANNOT_PERFORM_ACTION']] — Error code indicating a per-record permission denial. Possible values: **CANNOT_PERFORM_ACTION** - The current user does not have permission to change ownership of this record.
        - `details` (object) **REQ** — Additional context identifying the record for which the action was denied.
          - `id` (string) [maxLen=19] — The ID of the record for which the action was denied.
          - `index` (integer/int32) — The zero-based index of the record in the request.
        - `message` (string) **REQ** [maxLen=500] — Error message describing why the action was denied for this record.
        - `status` (string) **REQ** [enum=['error']] — Response status. Possible values: **error** - The operation did not complete.
