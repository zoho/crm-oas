...
**Operation:** `getAuditLogExportsById` — Get the Status of the Export Audit Log Job
...
**Parameters:**
- `id` (path, string, required) [maxLen=64]: Specify the unique ID of the audit log export job.
...
- **200**: Returns the details of the specified audit log export job. [application/json]
    > Represents the success response for retrieving an audit log export job by ID.
    - `audit_log_export` (array of object) [maxItems=100] — Represents the list containing the requested audit log export job.
...
- **204**: Returns an empty response when no export job data is available for the specified ID.
...
- **404**: The specified audit log export job ID does not exist. Resolution: A valid export job ID obtained from the Create Audit Log Export API must be provided. [application/json]
...
Possible values:
INVALID_URL_PATTERN - The specified audit log export job ID does not exist.
    - `details` (object) — Represents additional error details.
    - `message` (string) **REQ** [maxLen=256] — Represents the error message.
