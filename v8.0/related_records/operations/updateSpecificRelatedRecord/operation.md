...
**Operation:** `updateSpecificRelatedRecord` — Update a related record
...
> The request body must contain a **data** array with exactly one object.
  > Represents the request payload for updating a specific related record.
  - `data` (array of object) [minItems=1, maxItems=1] **REQ** — Array containing the record to update.
    - `id` (string/int64) **REQ** — Unique identifier of the record to update.
    additionalProperties: any

**Responses:**

- **200**: Returns the bulk operation response indicating the status of the update request. — Schema: `BulkOperationResponse` [application/json]
...
**Resolution:** Review the error code and message in the response to identify the specific issue. [application/json]
    > Contains one of the possible client error responses for invalid module name, unsupported module, invalid data, invalid request method, authorization failure, or mandatory field not found.
    oneOf:
