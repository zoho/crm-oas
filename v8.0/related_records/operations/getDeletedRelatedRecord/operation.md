...
**Operation:** `getDeletedRelatedRecord` — List deleted related records
...
- **200**: Returns the list of deleted related records for the specified parent record, along with pagination metadata. [application/json]
    > Represents the response for deleted related records, including the record references and pagination metadata.
    - `data` (array of object) [maxItems=200] **REQ** — Array of deleted related record references.
      - `id` (string) **REQ** [maxLen=20, pattern=^[0-9]+$] — Unique identifier of the deleted record.
      - `$member_info` (object) — Metadata about the related record.
        - `id` (string) [maxLen=20, pattern=^[0-9]+$] — ID of the member info.
        - `Created_Time` (string/date-time) — Represents the creation date and time of the record.
    ...
    - `info` (object) **REQ** — Pagination metadata for deleted records.
...
- **204**: No deleted related records were found for the specified parent record.
...
- **404**: The specified parent record does not exist.
**Resolution:** The **parentRecord** path parameter value must refer to an existing record. [application/json]
