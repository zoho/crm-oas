...
**Operation:** `postAddTagsWithId` — Add Tags to a Record
...
- `over_write` (query, boolean, optional): If true, replaces all existing tags on the record with the specified tags. If false or omitted, the specified tags are appended to the existing tags.
...
**Request Body** — application/json `PostaddtagsRequest`
> Request body containing the tags to add to the record.
  > Request body schema for adding tags to records.
  - `ids` (string) [maxLen=102, enum=['["3060320000002538026"]'], nullable] — JSON array string of record IDs to associate tags with. Use the [Get Records API](record.yaml#$.paths./module.get) to retrieve record IDs.

  - `over_write` (boolean) [nullable] — If true, replaces all existing tags. If false, appends tags to existing ones.
  - `tags` (array of object `TagsNested`) [maxItems=3] **REQ** — Array of tags to associate with the records (Required)
    schema: `TagsNested`
...
- **200**: OK - Tags added successfully. — Schema: `PostaddtagsResponse200` [application/json]
    > Success response when tags are associated successfully to records.
    schema: `PostaddtagsResponse200`
    - `ids` (string) [maxLen=102] — Record IDs that were processed
    - `over_write` (boolean) — Indicates whether tags were replaced or appended
    ...
    - `success_count` (string) [maxLen=255] — Number of records that were successfully updated
    - `tags` (string) [maxLen=255] — Root key for tags
...
- **400**: Bad Request - The request cannot be processed due to invalid syntax or validation errors. Review the error details and correct the issues before retrying. [application/json]
    ...
    oneOf:
      - `RecordLockedError` — Error indicating that the record is locked and cannot be modified.
      ...
      - `InvalidRequestMethodError` — Error indicating that the HTTP request method is invalid.
        ...
        - `status` (string) **REQ** [enum=['error']] — Error status
      - `TagErrorItemResponse` — Error object for individual tag failures.
        - `code` (string) **REQ** [enum=['INVALID_DATA']] — Error code
        - `message` (string) **REQ** [enum=['Invalid data', 'Invalid data type', 'invalid data']] — Error message
        - `status` (string) **REQ** [enum=['error']] — Error status
      ...
      - `TagInvalidDataTypeError` — Error indicating that a tag field has an invalid data type.
        ...
        - `status` (string) **REQ** [enum=['error']] — Error status
      - `MappingMismatchErrorResponse` — Error indicating a mapping mismatch between fields.
        - `code` (string) **REQ** [enum=['MAPPING_MISMATCH']] — Error code
        - `details` (object) **REQ** — Error details with validation information
          - `mapped_field` (object) — Field mapping details
            - `api_name` (string) [maxLen=255] — API name of the mapped field
            - `json_path` (string) [maxLen=1000] — JSON path to the mapped field
          - `api_name` (string) **REQ** [maxLen=255] — API name of the field with mapping mismatch
          - `json_path` (string) **REQ** [maxLen=1000] — JSON path to the field with mapping mismatch
        - `message` (string) **REQ** [maxLen=255] — Error message
        - `status` (string) **REQ** [enum=['error']] — Error status
        - `tags` (array of object) [maxItems=25] **REQ** — Array of error objects for tag field validation failures
          oneOf:
            ...
            - `TagFieldMaxLengthError` — Error indicating that a tag field exceeds the maximum allowed length.
              ...
              - `status` (string) **REQ** [enum=['error']] — Error status
        - `code` (string) **REQ** [enum=['FAILURE']] — Error code
        - `details` (object) **REQ** — Additional error context
        - `message` (string) **REQ** [enum=['permission denied to  update records']] — Human-readable error message
        - `status` (string) **REQ** [enum=['error']] — Error status
