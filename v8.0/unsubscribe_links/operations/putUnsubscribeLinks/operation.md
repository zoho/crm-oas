...
**Operation:** `putUnsubscribeLinks` — Update Unsubscribe Links
...
**Request Body** — application/json `UpdateUnsubscribeLinksRequest`
> Request body containing the unsubscribe links to update. The id field is mandatory for each link.
  > Request payload for bulk PUT requests to update unsubscribe link configurations.
  - `unsubscribe_links` (array of object `UnsubscribeLinkBulkUpdateItem`) [maxItems=1] **REQ** — Root key containing the unsubscribe links to update (required)
    schema: `UnsubscribeLinkBulkUpdateItem`
...
- **200**: Success - Unsubscribe link updated successfully — Schema: `UnsubscribeLinkModificationResultList` [application/json]
    > Collection of action status objects produced after modifying unsubscribe links via PUT requests.
    schema: `UnsubscribeLinkModificationResultList`
    - `unsubscribe_links` (array of object `UnsubscribeLinkActionStatus`) [maxItems=1] — Array containing the status of the modification operation for each link
...
- **400**: Bad Request - Invalid data provided. Check field types, dependencies, or refer to the Input JSON keys section in the help doc. [application/json]
    > Validation error response for the PUT request, covering invalid data type, missing required fields, empty array, and dependent field failures.
    anyOf:
      ...
      - `RequestBodyEmptyArray` — Error returned when the request body contains an empty array where data is expected.
        ...
        - `status` (string) **REQ** [enum=['error']] — Represents the status of the response.
        - `unsubscribe_links` (array of object) [maxItems=25] **REQ** — Represents the list of individual error objects for each unsubscribe link in the bulk update request.
          anyOf:
            - `UnsupportedPageTypeError` — Error returned when an unsupported value is provided for the page_type field. Supported values are 'standard' and 'custom'.
              ...
              - `status` (string) **REQ** [enum=['error']] — Represents the status of the response.
            - `InvalidUnsubscribeLinkIdInBody` — Error returned when an invalid unsubscribe link ID is provided in the request body that does not belong to your organization.
              - `code` (string) **REQ** [enum=['INVALID_DATA']] — Represents the error code for this response.
              - `details` (object) **REQ** — Contains additional validation information about the invalid unsubscribe link ID error.
                - `api_name` (string) **REQ** [maxLen=255] — The API name of the field with the invalid ID
                - `json_path` (string) **REQ** [maxLen=1000] — The JSON path of the field with the invalid ID
              ...
              - `status` (string) **REQ** [enum=['error']] — Represents the status of the response.
            - `NotAllowedOperationError` — Error returned when attempting to perform an operation on the default unsubscribe link, which is not allowed.
              - `code` (string) **REQ** [enum=['NOT_ALLOWED']] — Represents the error code for this response.
              - `details` (object) **REQ** — Contains additional validation information about the not-allowed operation error.
                - `api_name` (string) **REQ** [maxLen=255] — The API name of the field causing the error
                - `json_path` (string) **REQ** [maxLen=1000] — The JSON path of the field causing the error
              - `message` (string) **REQ** [enum=['The given Unsubscribe Link id is Default']] — Represents the error message describing the not-allowed operation.
              - `status` (string) **REQ** [enum=['error']] — Represents the status of the response.
            ...
            - `MandatoryFieldMissingError` — Error returned when a required mandatory field is missing from the request.
              ...
              - `status` (string) **REQ** [enum=['error']] — Represents the status of the response.
            - `InvalidDataTypeMismatchError` — Error returned when the data type provided does not match the expected type for a field.
              - `code` (string) **REQ** [enum=['INVALID_DATA']] — Represents the error code for this response.
              - `details` (object) **REQ** — Contains additional validation information about the data type mismatch error.
                - `expected_data_type` (string) **REQ** [maxLen=255] — The expected data type for this field
                - `api_name` (string) **REQ** [maxLen=255] — The API name of the invalid field
                - `json_path` (string) **REQ** [maxLen=1000] — The JSON path of the invalid field
              - `message` (string) **REQ** [enum=['invalid data']] — Represents the error message describing the data type mismatch.
              - `status` (string) **REQ** [enum=['error']] — Represents the status of the response.
